...
const windowsNoFocusScriptPath = () => resolve(hookStateDir(), WINDOWS_NO_FOCUS_SCRIPT_FILENAME);
...
const buildWindowsNoFocusLaunchScript = (command, args = []) => {
...
const writeWindowsNoFocusLaunchScript = (command, args = []) => {
  mkdirSync(hookStateDir(), { recursive: true });
  const scriptPath = windowsNoFocusScriptPath();
...
const buildWindowsBrowserStartCommand = () => {
  ...
  const startCommand = `start "" /min ${quoteCmd(browser.command)} ${browserArgs
    .map(quoteCmd)
    .join(' ')}`;
  return {
    ...browser,
    ...
    args: null,
    noFocusMethod: 'windows-wscript-minimized-no-focus',
    noFocusCommand: 'wscript.exe',
    noFocusArgs: null,
...
const prelaunchBrowserDetached = async (input) => {
  ...
  try {
    noFocusScriptPath = writeWindowsNoFocusLaunchScript(launch.browserCommand, launch.browserArgs);
    launch.noFocusArgs = ['//B', '//Nologo', noFocusScriptPath];
    launch.noFocusScriptPath = noFocusScriptPath;
  } catch (err) {
    launch.noFocusScriptError = err?.message || String(err);
  }
  if (!launch.args) {
    launch.method = launch.noFocusArgs ? launch.noFocusMethod : launch.directMethod;
    launch.command = launch.noFocusArgs ? launch.noFocusCommand : launch.directCommand;
    launch.args = launch.noFocusArgs || launch.directArgs;
  }
  const state = {
    source: 'hook',
    lastAttemptAt: now,
  ...
  writeHookDiagnostic({
    ...
    launch: {
      ...
      noFocusScriptPath: launch.noFocusScriptPath || null,
    },
  });
  ...
  let noFocusLaunch = null;
  if (launch.command !== launch.noFocusCommand && launch.noFocusArgs) {
    noFocusLaunch = await observeDetachedSpawn(
      launch.noFocusCommand,
      launch.noFocusArgs,
      DEFAULT_HOOK_LAUNCH_OBSERVE_MS,
    );
    if (noFocusLaunch.ok) {
      writeHookState({
        ...state,
        method: launch.noFocusMethod,
        command: launch.noFocusCommand,
        args: launch.noFocusArgs,
        restoreFocusLaunch: direct,
        launch: noFocusLaunch,
      });
      return;
    }
  }
  ...
  if (!fallback.ok) {
    ...
    nextState.error =
      ...
      noFocusLaunch?.error ||
      direct.error ||
      `exit ${fallback.exitCode ?? 'unknown'}`;
