import { buildProgressViewModel, normalizeProgressDisplayPercent, progressCreepCeiling, } from '../../core/progress-view-model.js';
export const SHARED_PROGRESS_CREEP_MAX_FRACTION = 0.85;
export const sharedProgressBarCurrent = (progress) => {
...
export const sharedComputeProgressMilestone = (progress) => {
    const view = buildProgressViewModel({
        sourceKind: 'gui-export',
        status: progress?.status || 'running',
        phase: progress?.phase || null,
...
export const sharedProgressCreepCeiling = (progress) => {
    const view = buildProgressViewModel({
        sourceKind: 'gui-export',
        status: progress?.status || 'running',
        phase: progress?.phase || null,
...
export const sharedShouldRunProgressCreep = (progress) => sharedProgressCreepCeiling(progress) > sharedComputeProgressMilestone(progress).base;
export const sharedNormalizeProgressDisplayPercent = ({ previousProgress = null, nextProgress, previousDisplayPercent = 0, }) => {
