...
(function () {
   ...
   * @param {Element} root
   */
  const stripAccessibilityLabels = (root) => {
   ...
   * @returns {string}
   */
  const buildDocument = ({ meta, turns }) => {
  ...
  // ============================================================
  function buildNotebookReturnPlan(options = {}) {
  ...
  function buildNotebookConversationPlan(options = {}) {
    const preserveContext = options.preserveContext !== false;
  ...
  function serializeConversationItem(item) {
  ...
  function createBatchExportSession({
    items,
    originalItem = null,
  ...
  function normalizeBatchExportSession(value) {
  ...
  const summarizeStrictLocator = ({ name = 'locator', count = 0 } = {}) => {
  ...
  const describeDomActionability = ({
    name = 'elemento',
    count = 1,
    attached,
    visible,
  ...
  const actionabilityTimeoutMessage = (snapshot = {}) => {
  ...
  class ChatIdentityError extends Error {
  ...
  const assertChatId = (value) => {
      const chatId = parseChatId(value);
      if (!chatId)
          throw new ChatIdentityError(value);
      return chatId;
  };
  const canonicalGeminiChatUrl = (chatId) => `https://gemini.google.com/app/${chatId}`;
  ...
  const createGeminiWebDomAdapter = ({ documentRef, locationHref = documentRef.location?.href || 'https://gemini.google.com/app', }) => {
  ...
  const navigationEvidence = (kind, confidence, warnings = []) => ({
      source: 'chat-dom',
      kind,
      confidence,
      warnings,
  });
  const blocked = (code, message, patch = {}) => ({
      ok: false,
      code,
      message,
  ...
  const rowMatchesChatId = (row, chatId) => row.chatId === chatId || parseChatId(row.url) === chatId;
  const targetChatId = (target) => parseChatId(target.chatId) ||
      parseChatId(target.url) ||
      parseChatId(target.row?.chatId) ||
      parseChatId(target.row?.url);
  const rowFromTarget = (rows, target, chatId) => {
      if (target.row)
          return target.row;
      ...
      if (chatId)
          return rows.find((row) => rowMatchesChatId(row, chatId)) || null;
      return null;
  };
  const currentRouteRow = (rows, target, chatId) => {
  ...
  const createNavigationEngine = ({ adapter, isBusy = () => false, openUrl, clickRow, waitForHydration, }) => ({
      async openChat(target) {
          ...
          if (requestedChatId && route.chatId === requestedChatId) {
              ...
              const hydrated = hydration.turnCount <= 0 && waitForHydration
          ...
          const hydrated = waitForHydration
              ...
              : { ok: true, turnCount: adapter.getHydrationState().turnCount };
          if (!hydrated.ok)
              return hydrated;
          return {
              ok: true,
  ...
  const hydrationDomProgressChanged = (before, after) => Boolean(before &&
      after &&
  ...
  const hydrationLayoutOnlyChanged = (before, after) => Boolean(before &&
      after &&
      !hydrationDomProgressChanged(before, after) &&
      numberValue(after.scrollHeight) > numberValue(before.scrollHeight) + 4);
  const hydrationSnapshotLooksLarge = (state) => numberValue(state?.turnDomCount) >= 80 ||
      numberValue(state?.containerCount) >= 80 ||
      numberValue(state?.scrollHeight) >= 30000;
  const hydrationConfirmationWaitMs = (state, { loadWaitMs, topSettleMs, smallSettleMs = 900 }) => {
      ...
      // ============================================================
  // Inlined from src/core/gemini-private-protocol.ts + src/browser/shared/private-api-content-fetch.ts (auto-generated — do not edit)
  // ============================================================
  ...
  const GEMINI_PRIVATE_BATCH_ENDPOINT = 'https://gemini.google.com/_/BardChatUi/data/batchexecute';
  const GEMINI_PRIVATE_RPC = {
      LIST_CHATS: 'MaZiqc',
      READ_CHAT: 'hNvQHb',
  };
  const PRIVATE_CHAT_ID_RE = /^c_([a-f0-9]{12,})$/i;
  const toGeminiPrivateChatId = (value) => {
      const text = String(value ?? '').trim();
  ...
  const buildGeminiPrivateReadChatPayload = (chatId, limit = 10) => {
  ...
  const buildGeminiPrivateListChatsPayload = ({ limit = 10, cursor = null, source = 0, } = {}) => [limit, null, [source, cursor, 1]];
  const buildGeminiPrivateBatchRequest = ({ rpcId, payload, session, requestId = 1, sourcePath = '/app', }) => {
  ...
  const decodeGeminiBatchExecuteResponseWithDiagnostics = (raw) => {
      const frames = [];
  ...
  const decodeGeminiBatchExecuteResponse = (raw) => decodeGeminiBatchExecuteResponseWithDiagnostics(raw).frames;
  ...
  const extractGeminiBatchRpcPayload = (frames, rpcId) => {
  ...
  const extractGeminiPrivateListChatIds = (payload) => {
  ...
  const normalizeGeminiPrivateReadChatSnapshot = ({ requestedChatId, payload, title, markdownRenderer, }) => {
  ...
  const checkGeminiPrivateSessionFromContent = async ({ fetchImpl = fetch, timeoutMs: rawTimeoutMs, } = {}) => {
      try {
  ...
  const readGeminiPrivateChatFromContent = async ({ chatId: rawChatId, title, fetchImpl = fetch, requestId = Date.now() % 100000, timeoutMs: rawTimeoutMs, }) => {
  ...
  const listGeminiPrivateChatsFromContent = async ({ fetchImpl = fetch, requestId = Date.now() % 100000, limit = 200, timeoutMs: rawTimeoutMs, } = {}) => {
      try {
          const timeoutMs = normalizeContentPrivateTimeoutMs(rawTimeoutMs, 30_000);
          const app = await fetchAppSessionInContent(fetchImpl, timeoutMs);
          if (!app.ok) {
      ...
      // ============================================================
  // Inlined from src/browser/shared/content-runtime-guard.ts (auto-generated — do not edit)
  // ============================================================
  const CONTENT_RUNTIME_BOOT_GRACE_MS = 15_000;
  const CONTENT_RUNTIME_HEARTBEAT_STALE_MS = 45_000;
  ...
  const classifyContentRuntimeHealth = ({ status, installedAt, nowMs = Date.now(), bootGraceMs = CONTENT_RUNTIME_BOOT_GRACE_MS, staleHeartbeatMs = CONTENT_RUNTIME_HEARTBEAT_STALE_MS, }) => {
  ...
  const transitionContentRuntimeGuard = (_state, event) => {
    ...
    const EXTENSION_PROTOCOL_VERSION = Number('2');
    ...
    const BUTTON_ICON_SVG = '<svg xmlns="http://www.w3.org/2000/svg" data-role="gm-md-export-download-icon" viewBox="0 -960 960 960" width="20" height="20" fill="currentColor" aria-hidden="true" focusable="false">' +
        ...
        '</svg>';
    const MENU_CHECK_ICON_SVG = '<svg xmlns="http://www.w3.org/2000/svg" data-role="gm-menu-check-icon" viewBox="0 -960 960 960" width="20" height="20" fill="currentColor" aria-hidden="true" focusable="false">' +
        '<path d="M382-240 154-468l57-57 171 171 367-367 57 57-424 424Z"/>' +
        '</svg>';
    const FOLDER_ICON_SVG = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" width="18" height="18" fill="currentColor" aria-hidden="true" focusable="false">' +
    ...
    const SCRIPT_VERSION = '0.8.60';
    const BUILD_STAMP = '20260604-0112';
    ...
    const findGeminiRightSectionPlacement = () => {
        ...
        for (const { el: topBar } of topBars) {
            ...
            const menuButton = rightSection.querySelector('[data-test-id="conversation-actions-menu-icon-button"]');
            const saveChatButton = rightSection.querySelector('#gemini-exporter');
            const legacyAnchor = shareButton?.closest('.buttons-container.share') ||
            ...
            let matchedBy = anchor
                ...
                : null;
            if (!anchor) {
                const containers = Array.from(rightSection.querySelectorAll(':scope > .buttons-container')).filter((el) => visibleRect(el));
    ...
    const findConversationActionPlacement = () => {
        ...
        if (geminiRightSection)
            return geminiRightSection;
        ...
        const candidates = controls.filter(({ rect, label }) => {
            ...
            const labelLooksRight = /(share|compartilhar|more|mais|options|opções|opcoes)/i.test(label || '') ||
                rect.left >= window.innerWidth * 0.72;
            if (ogb) {
                const ogbCenterY = ogb.rect.top + ogb.rect.height / 2;
                return (Math.abs(centerY - ogbCenterY) <= 34 &&
        ...
        const menu = candidates.find(({ label, el }) => el.getAttribute('aria-haspopup') === 'menu' ||
    ...
    const collectTopBarDiagnosticCandidates = ({ limit = 8, includeActions = false } = {}) => {
        ...
        const controls = includeActions
            ? Array.from(document.querySelectorAll('button,[role="button"],a[role="button"]'))
                .filter((el) => el.id !== BUTTON_ID && !el.closest?.(`#${BUTTON_SLOT_ID}`))
                .map((el) => ({ el, rect: visibleRect(el), label: controlLabel(el) }))
                .filter(({ rect }) => rect && (rect.top <= 120 || rect.left >= window.innerWidth * 0.55))
            : [];
        return {
    ...
    const buildTopBarDiagnostics = ({ includeCandidates = false } = {}) => {
        ...
        return {
            ...
            warning: !found && onConversationUrl
    ...
    const NOTEBOOK_CHAT_HISTORY_SELECTOR = 'project-chat-history, infinite-scroller.project-chat-history-scroller, .project-chat-history-container';
    ...
    const runtimeGuardDecision = transitionContentRuntimeGuard({ tag: 'checking' }, {
    ...
    const isExtensionContextInvalidatedError = (err) => /Extension context invalidated/i.test(String(err?.message || err || ''));
    ...
    const bridgeState = {
        ...
        windowId: null,
        isActiveTab: null,
        extensionVersion: '0.8.60',
        protocolVersion: EXTENSION_PROTOCOL_VERSION,
        buildStamp: '20260604-0112',
        lastError: null,
        heartbeatTimer: 0,
        polling: false,
        lastHeartbeatAt: 0,
    ...
    const scheduleDomWork = (reason = 'changed', flags = {}) => {
        ...
        if (reason) {
            ...
            if (scheduler.reasons.length > 12)
                scheduler.reasons.shift();
        }
        if (flags.topBar)
            scheduler.pending.add('topBar');
        if (flags.conversations)
            scheduler.pending.add('conversations');
        if (flags.modal)
            scheduler.pending.add('modal');
        if (scheduler.scheduled) {
            scheduler.skipped += 1;
            return;
        }
        scheduler.scheduled = true;
        const schedule = typeof pageWindow.requestAnimationFrame === 'function'
    ...
    const increaseBridgeEventBackoff = () => {
        bridgeState.eventsBackoffMs = Math.min(BRIDGE_EVENTS_MAX_BACKOFF_MS, bridgeBackoffWithJitter(bridgeState.eventsBackoffMs * 2, BRIDGE_EVENTS_BASE_BACKOFF_MS, BRIDGE_EVENTS_MAX_BACKOFF_MS));
        return bridgeState.eventsBackoffMs;
    };
    // --- metadata da página -----------------------------------------------
    const cleanText = (text) => normalizeWhitespace(String(text || '').replace(/\u200b/g, ''));
    ...
    const scrapeTitleFromDocument = (doc) => cleanText(doc.title.replace(/\s*[-—]\s*Gemini.*$/, ''));
    const scrapeTitle = () => scrapeTitleFromDocument(document);
    const scrapeModelFromDocument = (doc) => {
        const el = doc.querySelector('[data-test-id="bard-mode-menu-button"], bard-mode-switcher button');
    ...
    const buildNativeStyleProfile = () => buildGeminiNativeStyleProfile({ documentRef: document, isDark: isDarkTheme() });
    ...
    const showToast = (message, kind = 'info') => {
        ...
        if (!toast) {
            ...
            Object.assign(toast.style, {
                ...
                // com backdrop blur.
                boxShadow: '0 20px 48px rgba(0,0,0,0.34), 0 2px 6px rgba(0,0,0,0.26)',
                opacity: '0',
        ...
        const prefix = kind === 'error' ? '⚠️  ' : kind === 'success' ? '✅  ' : 'ℹ️  ';
    ...
    const withTimeoutValue = (promise, timeoutMs, fallbackValue) => new Promise((resolve) => {
        let settled = false;
        const finish = (value) => {
            if (settled)
                return;
    ...
    const supportsDirectoryPicker = () => typeof pageWindow.showDirectoryPicker === 'function';
    ...
    const hasOverflow = (el) => !!el && el.scrollHeight > el.clientHeight + 8;
    ...
    const findNotebookHistoryScroller = () => {
        ...
        for (const selector of NOTEBOOK_SCROLLER_SELECTORS) {
            for (const el of document.querySelectorAll(selector)) {
                if (!matches.has(el))
                    matches.set(el, selector);
            }
        }
        const all = Array.from(matches.keys());
        if (all.length === 0)
            return { wrapper: null, scroller: null };
        const wrapper = all.find((el) => el.tagName?.toLowerCase() === 'project-chat-history') ||
            all[all.length - 1];
        const scroller = all.find((el) => hasOverflow(el)) ||
    ...
    const getSidebarNav = () => {
        ...
        if (explicitSideNav)
            return explicitSideNav;
        return Array.from(document.querySelectorAll('[role="navigation"], mat-nav-list')).find((el) => !!el.querySelector?.(SIDEBAR_ITEM_SELECTOR)) || null;
    };
    const isSidebarOpen = () => {
        const sideNav = getSidebarNav();
        if (!sideNav)
            return false;
        if (sideNav.getAttribute('aria-hidden') === 'true')
            return false;
        if (sideNav.classList.contains('mat-drawer-closed'))
            return false;
    ...
    const findSidebarMenuButton = () => {
        ...
        return candidates
            ...
            const labelLooksRight = /(^menu$|main menu|show menu|open menu|expand menu|open navigation|show navigation|side navigation|sidebar|side nav|abrir navegação|abrir navegacao|abrir menu|expandir|barra lateral|menu principal|navigation)/i.test(label);
    ...
    const ensureSidebarOpen = async (options = {}) => {
        ...
        while (Date.now() - startedAt < timeoutMs) {
            ...
            if (await waitForSidebarOpen(Math.min(1200, remainingMs), pollMs))
                return true;
        }
        return isSidebarOpen();
    };
    const ensureSidebarOpenForCommand = async (command, settleMs) => {
        if (command.args?.ensureSidebar === false || isNotebookPage())
            return { ok: true };
    ...
    const getSidebarConversationElements = () => {
        ...
        if (primary.length > 0)
            return primary;
        return Array.from(document.querySelectorAll('[role="navigation"] [data-test-id="conversation"], [role="navigation"] [role="listitem"], .conversation-item'));
    };
    const getNotebookConversationElements = () => Array.from(document.querySelectorAll(NOTEBOOK_CHAT_ROW_SELECTOR)).filter((element) => element.querySelector('[data-test-id="navigate-to-recent-chat"]') ||
        element.querySelector('[data-test-id="chat-title"]'));
    const getConversationElementsForCurrentPage = () => isNotebookPage() && getNotebookConversationElements().length > 0
    ...
    const HIDDEN_CONVERSATION_STYLE_RE = /(?:^|;)\s*(?:display\s*:\s*none|visibility\s*:\s*(?:hidden|collapse))\b/i;
    const isHiddenConversationDomNode = (node) => {
        ...
        while (el && el.nodeType === 1) {
            ...
            if (HIDDEN_CONVERSATION_STYLE_RE.test(el.getAttribute?.('style') || ''))
                return true;
            const style = typeof pageWindow.getComputedStyle === 'function' ? pageWindow.getComputedStyle(el) : null;
            if (style &&
                (style.display === 'none' ||
                    style.visibility === 'hidden' ||
                    style.visibility === 'collapse')) {
    ...
    const conversationDomNodes = (doc = document) => Array.from(doc.querySelectorAll(CONVERSATION_TURN_DOM_SELECTOR)).filter((node) => !isHiddenConversationDomNode(node));
    ...
    const saveNotebookChatUrlCache = () => {
        try {
            pageWindow.localStorage?.setItem(NOTEBOOK_CHAT_URL_CACHE_STORAGE_KEY, JSON.stringify(loadNotebookChatUrlCache()));
        }
        catch {
    ...
    const saveBatchExportSession = (session) => {
        try {
            ...
            pageWindow.sessionStorage?.setItem(BATCH_EXPORT_SESSION_STORAGE_KEY, JSON.stringify(normalized));
            return normalized;
        }
        catch {
            return null;
    ...
    const getConversationTitleFromElement = (element, fallbackId) => {
        const title = cleanText(element.querySelector('.conversation-title, .title, [data-test-id="conversation-title"], [data-test-id="chat-title"]')?.textContent) ||
    ...
    const getNotebookConversationTitleFromElement = (element, fallbackId) => {
        const title = cleanText(element.querySelector('[data-test-id="chat-title"]')?.textContent) ||
            cleanText(element.querySelector('.chat-title')?.textContent) ||
            cleanText(element.getAttribute('aria-label')) ||
            cleanText(element.textContent).split('\n')[0];
        return title || fallbackId;
    };
    const getNotebookConversationSubtitleFromElement = (element) => cleanText(element.querySelector('[data-test-id="chat-subtitle"]')?.textContent);
    const getNotebookConversationTimestampFromElement = (element) => cleanText(element.querySelector('[data-test-id="chat-timestamp"]')?.textContent);
    ...
    const getNotebookChatIdFromElement = (element) => {
        if (notebookChatIdCache.has(element))
            return notebookChatIdCache.get(element);
        const chatId = extractNotebookChatIdFromAttributes(element) ||
    ...
    const conversationCacheKey = (item) => {
        ...
        if (/^[a-f0-9]{12,}$/i.test(chatId))
            return `chat:${chatId.toLowerCase()}`;
        if (item?.url && /\/app\/[a-f0-9]{12,}/i.test(item.url))
            return `url:${item.url}`;
        return `title:${item?.title || ''}:${item?.timestamp || ''}`;
    };
    const rememberSidebarConversationLinks = (items) => {
        for (const item of items) {
            if (!item || item.source === 'notebook')
                continue;
    ...
    const collectCachedSidebarConversationLinks = () => rememberSidebarConversationLinks(collectSidebarConversationLinks());
    const collectConversationLinks = () => {
        if (isNotebookPage()) {
            const notebookConversations = collectNotebookConversationLinks();
            if (notebookConversations.length > 0)
                return notebookConversations;
        }
        return collectCachedSidebarConversationLinks();
    };
    const collectBridgeConversationLinks = () => isNotebookPage()
        ? mergeConversationLists(collectCachedSidebarConversationLinks(), collectNotebookConversationLinks(), collectCurrentConversationLink())
    ...
    const collectConversationLinkSnapshot = () => {
        ...
        const modalConversations = notebookPage && notebookConversations.length > 0
    ...
    const refreshConversationState = () => {
        ...
        state.selectedChatIds = new Set(state.conversations
            .filter((item) => previousSelection.size > 0
    ...
    const conversationSearchText = (item) => [
        ...
        .join(' ')
        .toLowerCase();
    const conversationDisplayId = (item) => item.chatId ||
        item.timestamp ||
        (item.source === 'notebook' ? 'Conversa do caderno' : item.id);
    const currentListKind = () => isNotebookPage() && state.conversations.some((item) => item.source === 'notebook')
        ? 'notebook'
        : 'sidebar';
    const listEndText = () => {
    ...
    const listIdleStatusText = () => currentListKind() === 'notebook'
    ...
    const listEndStatusText = () => currentListKind() === 'notebook'
        ? 'Fim do caderno confirmado: não há mais conversas pra carregar.'
        : 'Fim do histórico confirmado no sidebar.';
    const listInconclusiveStatusText = () => currentListKind() === 'notebook'
    ...
    const hasConversationNode = (node) => {
        if (!(node instanceof Element))
            return false;
        return (node.matches?.('[data-test-id="conversation"]') ||
    ...
    const sidebarRootHasExtractableConversation = (root) => sidebarConversationItemsWithin(root).some((element) => !!getChatIdFromSidebarElement(element));
    const findSidebarHistoryScroller = () => {
        const roots = uniqueElements(Array.from(document.querySelectorAll(SIDEBAR_SCROLL_ROOT_SELECTOR)));
        const relevantRoots = roots.filter(sidebarRootHasExtractableConversation);
        if (relevantRoots.length === 0)
            return { wrapper: null, scroller: null, matchedBy: null };
        const preferredWrapper = relevantRoots.find((el) => el.tagName?.toLowerCase() === 'conversations-list') ||
            relevantRoots.find((el) => el.tagName?.toLowerCase() === 'mat-nav-list') ||
            relevantRoots.find((el) => el.tagName?.toLowerCase() === 'infinite-scroller') ||
            relevantRoots[0];
        const scroller = relevantRoots.find((el) => hasOverflow(el)) ||
    ...
    const startSidebarConversationObserver = () => {
        ...
        sidebarConversationObserver = new MutationObserver((mutations) => {
            if (mutations.some((mutation) => Array.from(mutation.addedNodes).some((node) => hasConversationNode(node)))) {
    ...
    const traceConfirmsStableTail = (trace) => trace.some((entry) => {
        if (entry.loaded === true || entry.phase !== 'confirm-end')
            return false;
        const beforeKnown = Number(entry.beforeKnown);
        const afterKnown = Number(entry.afterKnown);
        ...
        if (!entry.beforeTail || entry.beforeTail !== entry.afterTail)
            return false;
        if (entry.actionability?.receivingEvents === false)
            return false;
        return true;
    });
    const waitForSidebarConversationGrowth = (beforeCount, timeoutMs = 1800, beforeKnownCount = null) => new Promise((resolve) => {
        const conversationsList = isNotebookPage()
            ? findNotebookHistoryScroller().wrapper
            : findSidebarHistoryScroller().wrapper;
        if (!conversationsList) {
        ...
        const checkGrowth = () => {
            ...
            const currentKnownCount = beforeKnownCount === null ? null : collectBridgeConversationLinks().length;
    ...
    const triggerSidebarLoading = async (options = {}) => {
        ...
        pickScrollContainer(hasOverflow(sidebarList) ? sidebarList : null, 'conversations-list overflow');
        pickScrollContainer(findScrollableParent(sidebarList), 'conversations-list scroll-parent');
        const navigation = document.querySelector('[role="navigation"]');
        pickScrollContainer(hasOverflow(navigation) ? navigation : null, 'navigation overflow');
        pickScrollContainer(findScrollableParent(navigation), 'navigation scroll-parent');
        pickScrollContainer(findScrollableParent(document.querySelector('[data-test-id="conversation"]')), 'conversation scroll-parent');
        if (!scrollContainer) {
            return {
                loaded: false,
                scroller: null,
        ...
        const growthPromise = waitForSidebarConversationGrowth(before, loadOptions.growthTimeoutMs, beforeKnown);
    ...
    const loadMoreConversations = async (attempts = 2, options = {}) => {
        ...
        const endFailureThreshold = Math.max(3, Math.min(20, Number(loadOptions.endFailureThreshold || 3)));
        ...
        try {
            ...
            if (!loaded &&
                ...
                const confirmation = await triggerSidebarLoading({
                    ...loadOptions,
                    growthTimeoutMs: Math.max(loadOptions.growthTimeoutMs, loadOptions.confirmEndGrowthTimeoutMs),
                    postLoadSettleMs: Math.max(loadOptions.postLoadSettleMs, loadOptions.confirmEndPostLoadSettleMs),
                });
            ...
            const confirmedStableBottom = !loaded &&
                scrolledToBottom &&
                trace.some((entry) => entry.phase === 'confirm-end' &&
                    entry.loaded !== true &&
                    Number(entry.beforeKnown) === Number(entry.afterKnown) &&
                    scrollInfoIsNearBottom(entry.scrollAfter));
            const requiredEndFailures = confirmedStableBottom || confirmedStableTail ? 1 : endFailureThreshold;
    ...
    const listLegacyUiNodes = () => LEGACY_UI_SELECTORS.flatMap((selector) => Array.from(document.querySelectorAll(selector)).map((el) => ({
        selector,
        tag: el.tagName.toLowerCase(),
    ...
    const sidebarDiagnostics = () => {
        ...
        return {
            ...
            scrollRoots: roots.slice(0, 12).map((root, index) => ({
                ...
                childExtractableConversationItemCount: sidebarConversationItemsWithin(root).filter((el) => !!getChatIdFromSidebarElement(el)).length,
    ...
    const debugSnapshot = ({ includeDomDiagnostics = true } = {}) => {
        const { modalConversations, bridgeConversations, } = collectConversationLinkSnapshot();
        return {
            version: '0.8.60',
            protocolVersion: EXTENSION_PROTOCOL_VERSION,
            buildStamp: '20260604-0112',
    ...
    const installContentScriptMessageListener = () => {
        ...
        chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
            if (message?.type !== 'gemini-md-export/content-ping')
                return false;
    ...
    const ARTIFACT_LAUNCHER_STRONG_RE = /(artifact|artefato|immersive|gemini-code|usercontent\.goog|scf\.usercontent|interactive|interativo)/i;
    const ARTIFACT_LAUNCHER_WEAK_RE = /(preview|prévia|previsuali|visualizar|abrir|open|launch|run|executar|app|aplicativo|canvas|code|c[oó]digo)/i;
    const ARTIFACT_LAUNCHER_NEGATIVE_RE = /(gemini-md-export|exportar markdown|share|compartilhar|more options|open menu|menu|fechar|close|copy|copiar|editar|edit|like|dislike|regenerar|retry|ouvir|listen|download)/i;
    ...
    const frameAllowTokens = (el) => String(el?.getAttribute?.('allow') || '')
        .split(';')
        .map((part) => part.trim())
        .filter(Boolean);
    const classifyArtifactFrameSource = (el) => {
        ...
        if (srcdoc)
            return { srcKind: 'srcdoc', host: null, pathname: null };
        if (!src)
            return { srcKind: 'empty', host: null, pathname: null };
        if (/^data:text\/html/i.test(src))
            return { srcKind: 'data_html', host: null, pathname: null };
        if (/^data:/i.test(src))
            return { srcKind: 'data', host: null, pathname: null };
        if (/^blob:/i.test(src))
            return { srcKind: 'blob', host: null, pathname: null };
        try {
    ...
    const artifactKindForFrame = (el, sourceInfo) => {
        ...
        if (/usercontent\.goog/i.test(src))
            return 'usercontent_frame';
        return 'iframe';
    };
    ...
    const artifactFrameElements = () => Array.from(document.querySelectorAll(ARTIFACT_IFRAME_SELECTOR)).filter(isArtifactFrameCandidate);
    const elementArtifactText = (el) => [
        ...
        .filter(Boolean)
        .join(' ');
    const normalizeArtifactLauncherElement = (el) => el?.closest?.('button,[role="button"],a[href],a[role="button"]') || el;
    ...
    const closeOpenedArtifactSurface = async (previousActiveElement) => {
        const closeButton = Array.from(document.querySelectorAll([
            ...
            '[mat-dialog-close]',
        ].join(','))).find((el) => visibleRect(el) && !el.closest?.(`#${MODAL_ID},#${MENU_ID}`));
        if (closeButton) {
            closeButton.click();
        }
        else {
    ...
    const openArtifactLaunchersForDiagnosis = async (launchers, options = {}) => {
        ...
        return {
            ...
            openedFrameCount,
            close: async () => {
                if (!clicked.some((item) => item.ok) || options.closeOpenedLaunchers === false)
                    return null;
                try {
    ...
    const recommendedProbeForArtifact = (sourceInfo, parentProbe) => {
        if (parentProbe?.readable)
            return 'parent_dom';
        if (sourceInfo?.srcKind === 'srcdoc' || sourceInfo?.srcKind === 'data_html')
            return 'inline_html';
        if (sourceInfo?.srcKind === 'remote_usercontent_goog' ||
            sourceInfo?.srcKind === 'remote_googleusercontent_com') {
            return 'chrome_scripting_frame';
        }
        if (sourceInfo?.srcKind === 'blob')
            return 'live_blob_fetch';
        return 'fallback';
    };
    const artifactExportRecommendation = (item) => {
        ...
        if (item.recommendedProbe === 'chrome_scripting_frame')
            return 'reload_extension_or_frame_probe';
        if (item.recommendedProbe === 'live_blob_fetch')
            return 'try_blob_fetch';
    ...
    const inspectArtifactElement = (el, { includeHtml = false } = {}) => {
        ...
        const item = {
            ...
            recommendedProbe,
            extensionFrameProbePossible: recommendedProbe === 'chrome_scripting_frame' || sourceInfo.srcKind === 'remote_same_origin',
            frameProbe: null,
            htmlExtractable: parentProbe.readable ||
    ...
    const summarizeArtifactItems = (items) => {
        ...
        items.forEach((item) => {
            ...
            if (item.htmlExtractable)
                htmlExtractable += 1;
            if (item.frameProbe?.htmlReadable)
                frameProbeReadable += 1;
            if (item.parentDomReadable)
                parentDomReadable += 1;
            if (!item.htmlExtractable)
                opaque += 1;
        });
    ...
    const mergeFrameProbeResults = (items, frameProbeResult) => {
        if (!frameProbeResult?.ok || !Array.isArray(frameProbeResult.frames))
            return items;
        const byUrl = new Map();
    ...
    const inspectArtifactDom = async (options = {}) => {
        ...
        const payload = {
            ...
            launchers,
            items,
            nextAction: items.some((item) => item.htmlExtractable)
                ? {
                ...
                : items.some((item) => item.recommendedProbe === 'chrome_scripting_frame')
                    ? {
                        ...
                        message: 'Há iframe em usercontent.goog, mas o probe de frame não leu HTML. Recarregue a extensão se a permissão nova ainda não estiver ativa.',
                    }
                    : {
                        code: 'fallback_only',
    ...
    const getOrCreateChatClientId = () => getOrCreateBridgeClientId({
    ...
    const withTimeout = (promise, timeoutMs, message) => new Promise((resolve, reject) => {
        const timeoutId = setTimeout(() => reject(new Error(message || 'timeout')), timeoutMs);
    ...
    const extensionSendMessage = (message, { timeoutMs = 12000 } = {}) => withTimeout(new Promise((resolve, reject) => {
        if (!isExtensionContext) {
            resolve(null);
            return;
        }
    ...
    }), timeoutMs, `Tempo esgotado ao falar com a extensão (${message?.type || 'mensagem'}).`);
    const extensionSendMessageWithRetry = async (message, { timeoutMs = 2500, attempts = 2, retryDelayMs = 180 } = {}) => {
        let lastError = null;
        ...
        const error = new Error(`${lastError?.message || 'Não consegui falar com a extensão.'} ` +
    ...
    const shouldUseContentPrivateApiFallback = (response, err = null) => {
        ...
        return (!response ||
            ...
            /Tempo esgotado ao falar com a extensão|message channel closed|Extension context invalidated/i.test(message));
    };
    ...
    const applyTabClaimTitleFallback = (claim) => {
        const label = String(claim?.label || TAB_CLAIM_DEFAULT_LABEL).replace(/[\[\]]/g, '').trim() ||
    ...
    const releaseTabClaimViaExtension = async ({ claimId, reason } = {}) => extensionSendMessage({
        type: 'gemini-md-export/release-tab-claim',
    ...
    const nativeBridgeRequest = async (path, { method = 'GET', payload, timeoutMs = 10000 } = {}) => {
    ...
    const bridgeRequest = async (path, { method = 'GET', payload, timeoutMs = 10000 } = {}) => {
        ...
        const response = await fetchWithTimeout(`${BRIDGE_BASE_URL}${path}`, {
            method,
            headers: payload
    ...
    const setBridgeOutputDir = (outputDir) => {
        ...
        try {
            if (state.bridgeOutputDir) {
                pageWindow.localStorage?.setItem(BRIDGE_OUTPUT_DIR_STORAGE_KEY, state.bridgeOutputDir);
            }
            else {
    ...
    const pickBridgeOutputDir = async () => {
        ...
        if (!response?.ok || !response.outputDir) {
            ...
            if (/-128\b|user canceled|cancelled|canceled|cancelado|annul|abgebrochen/i.test(reason)) {
                return { status: 'cancelled' };
            }
            return { status: 'error', reason };
        }
    ...
    const writeBrowserFile = async (directoryHandle, filename, file) => {
        ...
        if (file.contentBase64) {
            ...
            await writable.write(new Blob([bytes], { type: file.mimeType || 'application/octet-stream' }));
        }
        else {
    ...
    const buildBridgeSnapshotPayload = () => {
        const { modalConversations, bridgeConversations, sidebarConversations, notebookConversations, } = collectConversationLinkSnapshot();
    ...
    const mediaAssetExtensionFor = (mimeType, source) => {
        ...
        if (type === 'image/svg+xml')
            return 'svg';
        const sourceExt = String(source || '').match(/\.([a-z0-9]{2,5})(?:[?#].*)?$/i)?.[1];
        if (sourceExt && /^(?:avif|gif|jpe?g|png|svg|webp)$/i.test(sourceExt)) {
            return sourceExt.toLowerCase() === 'jpeg' ? 'jpg' : sourceExt.toLowerCase();
        }
        return 'png';
    };
    const escapeMarkdownAlt = (value) => normalizeWhitespace(value || 'Imagem do Gemini')
        .replace(/\\/g, '\\\\')
        .replace(/\]/g, '\\]');
    ...
    const shouldPrepareImageBeforeFetch = (source) => !/^(?:blob|data):/i.test(String(source || ''));
    ...
    const shouldCaptureRenderedMediaFirst = (source) => isExtensionContext && isProtectedGoogleMediaSource(source);
    ...
    const imageElementToAsset = async (img) => {
        if (!(img instanceof HTMLImageElement))
            return null;
        if (!img.complete) {
            await withTimeout(img.decode?.() || Promise.resolve(), 1500, 'Tempo esgotado ao decodificar imagem renderizada.');
        }
    ...
    const captureRenderedImageAsset = async (img) => {
        if (!isExtensionContext || !(img instanceof HTMLImageElement))
            return null;
        if (!(await waitForImageElementReady(img, MEDIA_IMAGE_READY_TIMEOUT_MS)))
            return null;
    ...
    const mediaSourcesFromRoot = (root) => {
        ...
        root.querySelectorAll('[style]').forEach((el) => {
            cssUrlValues(el.style?.backgroundImage || el.getAttribute?.('style')).forEach((source) => pushUnique(sources, source));
        });
        return sources;
    };
    const findLightboxTrigger = (img) => {
        if (!(img instanceof HTMLElement))
            return null;
        return (img.closest('.image-button, .preview-image-button, button, [role="button"]') ||
            mediaReplacementTarget(img, document.body));
    };
    const lightboxRoots = () => Array.from(document.querySelectorAll(LIGHTBOX_ROOT_SELECTOR)).filter((el) => !el.closest?.(`#${MODAL_ID}`));
    const findOpenedLightbox = (previousRoots) => {
        const before = previousRoots || new Set();
        const roots = lightboxRoots().filter(visibleElement);
        return (roots.find((root) => !before.has(root) && mediaSourcesFromRoot(root).length > 0) ||
    ...
    const fetchImageAssetViaLightbox = async (img, originalSource) => {
        ...
        try {
            trigger.click();
            const root = await waitForCondition(() => findOpenedLightbox(previousRoots), MEDIA_LIGHTBOX_TIMEOUT_MS);
            if (!root)
                return null;
            try {
    ...
    const fetchImageAsset = async (source) => {
        ...
        if (/^data:/i.test(source))
            return parseDataUrlAsset(source);
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        throw new Error(`bridge: ${bridgeErr?.message || String(bridgeErr)}; page: ${pageErr?.message || String(pageErr)}; background: ${backgroundErr?.message || String(backgroundErr)}`);
                    }
                }
            }
        }
        if (shouldFetchViaBackgroundFirst(source)) {
            try {
                ...
                try {
                    ...
                    throw new Error(`background: ${backgroundErr?.message || String(backgroundErr)}; page: ${pageErr?.message || String(pageErr)}`);
                }
            }
        }
        try {
            ...
            if (isExtensionContext) {
                ...
                try {
                    ...
                    throw new Error(`page: ${err?.message || String(err)}; background: ${backgroundErr?.message || String(backgroundErr)}`);
                }
            }
            throw err;
        }
    ...
    const collectMediaCandidatesForTurn = (turnNode) => {
        ...
        Array.from(turnNode.querySelectorAll(MEDIA_ASSET_SELECTOR)).forEach((el) => {
            ...
            if (replaced.has(target) || !turnNode.contains(target))
                return;
            const placeholder = mediaPlaceholderFor(el);
            if (!placeholder)
                return;
            ...
            for (const node of nodes) {
                ...
                for (const candidate of candidates) {
                    ...
                    try {
                            ...
                            try {
                                ...
                                if (!asset?.contentBase64) {
                                    try {
                                        ...
                                        finalFetchErr = new Error(`${fetchErr?.message || String(fetchErr)}; ${lightboxErr?.message || String(lightboxErr)}`);
                                    }
                                }
                                if (!asset?.contentBase64)
                                    throw finalFetchErr;
                        ...
                        const filename = `assets/${chatId}/${rolePrefix}-${String(exportedTurnIndex + 1).padStart(2, '0')}-image-${String(mediaIndex).padStart(2, '0')}.${ext}`;
    ...
    const tabOperationLabel = (command) => {
        ...
        if (type === 'load-more-conversations')
            return 'carregando histórico';
        if (type === 'list-conversations')
            return 'listando conversas';
        if (type === 'get-current-chat')
            return 'exportando conversa atual';
        if (type === 'get-chat-by-id')
            return 'exportando conversa por ID';
        if (type === 'open-chat')
            return 'abrindo conversa';
        return type;
    };
    ...
    const activeTabOperationCancelRequested = () => Boolean(state.activeTabOperation?.cancelRequestedAt ||
    ...
    const maybeReleaseClaimAfterTabOperation = async (command, result, startedAt) => {
        ...
        const terminal = result?.reachedEnd === true ||
    ...
    const reportTabBrokerState = async (reason = 'heartbeat', { force = false } = {}) => {
        if (!isExtensionContext)
            return null;
        const now = Date.now();
        if (!force && now - lastTabBrokerReportAt < TAB_BROKER_REPORT_MIN_MS)
            return null;
    ...
    const refreshExtensionContextSoon = ({ force = false, reason = 'background' } = {}) => {
        if (!isExtensionContext || extensionContextRefreshInFlight)
            return;
    ...
    const reportTabBrokerStateSoon = (reason = 'heartbeat', { force = false } = {}) => {
        if (!isExtensionContext || tabBrokerReportInFlight)
            return;
    ...
    const runWithTabOperationBackpressure = async (command, fn) => {
        ...
        if (state.activeTabOperation) {
            ...
            return {
                ...
                error: 'Esta aba do Gemini já está ocupada com outro comando pesado. Aguarde terminar antes de enviar outro.',
                activeOperation: activeTabOperationSummary(),
            };
        }
        const abortController = new AbortController();
        const operationId = command.args?.operationId ||
        ...
        try {
            ...
            await maybeReleaseClaimAfterTabOperation(command, {
                ok: false,
                error: err?.message || String(err),
                code: err?.code || null,
            }, operationStartedAt);
    ...
    const findConversationForBridgeCommand = (args = {}) => {
        ...
        const requestedChatId = stripGeminiConversationPrefix(args.chatId) ||
        ...
        if (args.title) {
            ...
            const exact = conversations.find((conversation) => String(conversation.title || '').toLowerCase() === query);
            if (exact)
                return exact;
            const partial = conversations.find((conversation) => String(conversation.title || '').toLowerCase().includes(query));
            if (partial)
                return partial;
        }
        if (requestedChatId) {
    ...
    const sharedTabCommands = createSharedTabCommandHandlers({
        ...
        getExtensionInfo: async () => {
            ...
            try {
                const ping = await extensionSendMessageWithRetry({ type: 'GET_EXTENSION_INFO' }, { timeoutMs: 1200, attempts: 1, retryDelayMs: 0 });
                ...
                if (response?.buildStamp)
                    bridgeState.buildStamp = response.buildStamp;
                return {
                    ...localInfo(),
                    ...(response || {}),
                    contentScript: true,
        ...
        afterReleaseByTabId: async ({ command, response, requestedTabId }) => {
            ...
            const claimMatches = !command.args?.claimId ||
    ...
    const hasExplicitBrowserCommandIntent = (command) => command.args?.explicit === true ||
        command.args?.explicitBrowserSideEffect === true ||
        command.args?.browserSideEffectExplicit === true ||
        command.args?.force === true;
    const hasBridgeBrowserAuthorityLease = (command) => typeof command.args?.browserAuthorityLeaseId === 'string' &&
    ...
    const captureHostStylesProbe = () => {
        ...
        const sidebarItem = first('side-nav-action-button.is-selected, .conversations-list .selected, mat-list-item.mdc-list-item--selected, [data-test-id="conversation"].is-selected, gem-nav-list-item.is-selected') || first('gem-nav-list-item');
    ...
    const executeBridgeCommand = async (command, operationContext = {}) => {
        ...
        if (sharedResult !== undefined)
            return sharedResult;
        if (browserSideEffectCommands.has(String(command.type || '')) &&
        ...
        if (command.type === 'cancel-active-operation') {
            ...
            const hasRequestedOperationId = Object.prototype.hasOwnProperty.call(command.args || {}, 'operationId') &&
                command.args?.operationId !== null;
            const requestedOperationId = hasRequestedOperationId ? String(command.args.operationId) : null;
            if (hasRequestedOperationId &&
            ...
            if (state.activeTabOperation.abortController) {
                state.activeTabOperation.abortController.abort(state.activeTabOperation.cancelReason || 'bridge-command');
            }
        ...
        if (command.type === 'get-extension-info') {
            const ping = await extensionSendMessageWithRetry({ type: 'GET_EXTENSION_INFO' }, { timeoutMs: 3500, attempts: 2, retryDelayMs: 200 });
            const response = ping.response;
            if (response?.ok && response.protocolVersion !== undefined) {
                bridgeState.protocolVersion = response.protocolVersion;
            }
        ...
        if (command.type === 'private-api-read-chat') {
            const requestedChatId = normalizeExpectedChatId(command.args || {}) || extractChatId(location.pathname);
        ...
        if (command.type === 'activate-tab' || command.type === 'activate-browser-tab') {
            ...
            if (response?.isActiveTab !== undefined &&
        ...
        if (command.type === 'load-more-conversations') {
            ...
            const endFailureThreshold = Math.max(3, Math.min(20, Number(command.args?.endFailureThreshold || loadOptions.endFailureThreshold || 3)));
            const timeoutMs = Math.max(500, Math.min(30_000, Number(command.args?.timeoutMs || (command.args?.fastMode ? 3500 : 8000))));
            const startedAt = Date.now();
            ...
            for (let round = 0; round < maxRounds; round += 1) {
                ...
                if (currentCount >= targetCount || state.reachedSidebarEnd)
                    break;
                ...
                const loadedResult = await withTimeoutValue(loadMoreConversations(attempts, loadOptions), remainingMs, timeoutSentinel);
                ...
                if (roundTimedOut)
                    break;
                if ((afterCount <= previousCount || !loaded) && !untilEnd)
                    break;
                if ((afterCount <= previousCount || !loaded) && noGrowthRounds >= endFailureThreshold)
                    break;
                await sleep(loadOptions.retryPauseMs);
            ...
            return {
                ...
                conversations: command.args?.includeConversations === false ? undefined : collectBridgeConversationLinks(),
                modalConversations: command.args?.includeConversations === false ||
                    command.args?.includeModalConversations === false
                    ? undefined
                    : collectConversationLinks(),
                snapshot: command.args?.includeSnapshot === false
                    ? undefined
                    : debugSnapshot({
                        includeDomDiagnostics: command.args?.includeDomDiagnostics === true,
                    }),
        ...
        if (command.type === 'get-chat-by-id') {
            ...
            if (command.args?.returnToOriginal !== false &&
                originalItem &&
            ...
            else if (command.args?.returnToOriginal !== false &&
    ...
    const getScrollTarget = (scroller, doc, win) => scroller === doc.documentElement || scroller === doc.scrollingElement ? win : scroller;
    const getScrollTop = (target, win) => target === win ? win.scrollY || win.pageYOffset || 0 : target.scrollTop || 0;
    ...
    const normalizeHydrationOptions = (options = {}) => {
        const maxTotalMs = boundedHydrationMs(options.maxTotalMs ?? options.hydrationMaxTotalMs ?? options.hydrationTimeoutMs, HYDRATION_MAX_TOTAL_MS, 5000, 30 * 60 * 1000);
        return {
            loadWaitMs: boundedHydrationMs(options.loadWaitMs ?? options.hydrationLoadWaitMs, HYDRATION_LOAD_WAIT_MS, 500, 30_000),
            topSettleMs: boundedHydrationMs(options.topSettleMs ?? options.hydrationTopSettleMs, HYDRATION_TOP_SETTLE_MS, 1000, 60_000),
            stallTimeoutMs: boundedHydrationMs(options.stallTimeoutMs ?? options.hydrationStallTimeoutMs, HYDRATION_STALL_TIMEOUT_MS, 5000, maxTotalMs),
            maxTotalMs,
            maxAttempts: Math.max(1, Math.min(5000, Number(options.maxAttempts ?? options.hydrationMaxAttempts ?? HYDRATION_MAX_ATTEMPTS) ||
    ...
    const firstConversationSignature = (scroller, doc) => {
        const first = scroller?.querySelector?.(CONVERSATION_CONTAINER_SELECTOR) ||
    ...
    const conversationHydrationChanged = (before, after) => hydrationDomProgressChanged(before, after);
    const topHydrationConfirmationMs = (state, { loadWaitMs, topSettleMs }) => hydrationConfirmationWaitMs(state, {
        loadWaitMs,
        topSettleMs,
    ...
    const hydrateConversationToTop = async (doc, win, options = {}) => {
        const { loadWaitMs, topSettleMs, stallTimeoutMs, maxTotalMs, maxAttempts, } = normalizeHydrationOptions(options);
        ...
        const hydrationAbortMessage = 'Exportação cancelada antes de terminar a hidratação da conversa.';
        ...
        while (attempts < maxAttempts) {
            ...
            if (beforeTop <= 2 && attempts > 1) {
                ...
                const finalCheck = await waitForHydrationChange(beforeState, topHydrationConfirmationMs(beforeState, { loadWaitMs, topSettleMs }));
            ...
            if (growth.changed) {
                ...
                await nudgeConversationTop(scrollTarget, scrollElement, doc, win);
                const confirmation = await waitForHydrationChange(growth.state, topHydrationConfirmationMs(growth.state, { loadWaitMs, topSettleMs }));
    ...
    const waitForChatToLoad = async (targetChatId, options = {}) => {
        ...
        const sameSignatureGraceMs = Math.max(0, Number(options.sameSignatureGraceMs ?? SAME_SIGNATURE_ACCEPT_GRACE_MS) ||
        ...
        return waitFor(() => {
            ...
            const acceptedSameSignature = Boolean(previousSignature && !changedFromPrevious && previousChatId !== chatId) &&
            ...
            if (chatId !== normalizedTargetChatId)
                return false;
            ...
            if (previousSignature && !changedFromPrevious && !acceptedSameSignature)
                return false;
            return lastState;
        }, {
    ...
    const getSidebarConversationById = (itemId) => getSidebarConversationElements().find((element) => getChatIdFromSidebarElement(element) === itemId);
    const getNotebookConversationById = (itemId) => getNotebookConversationElements().find((element, index) => buildNotebookConversationId(element, index) === itemId);
    const waitForNotebookToLoad = async (item) => {
        await waitFor(() => {
            if (!isNotebookPage())
                return false;
            if (item.notebookId && currentNotebookId() !== item.notebookId)
                return false;
            const rows = getNotebookConversationElements();
            if (Number.isInteger(item.rowIndex))
                return rows.length > item.rowIndex;
    ...
    const sameNotebookConversation = (candidate, item) => {
        if (!candidate || !item)
            return false;
        if (item.cacheKey && candidate.cacheKey === item.cacheKey)
            return true;
        ...
        if (itemChatId && candidateChatId && itemChatId === candidateChatId)
            return true;
    ...
    const refreshNotebookConversationItem = (item) => {
        if (!isNotebookPage())
            return item;
        const fresh = collectNotebookConversationLinks().find((candidate) => sameNotebookConversation(candidate, item));
        if (!fresh)
            return item;
        const knownChatId = item.chatId || fresh.chatId || '';
        const knownAppUrl = (item.url && String(item.url).includes('/app/') ? item.url : '') ||
    ...
    const navigateDirectlyToNotebookPage = async (item, notebookUrl, options = {}) => {
        ...
        if (await waitForNotebookReturn(item, 12000))
            return true;
    ...
    const returnToNotebookPage = async (item, options = {}) => {
        ...
        const notebookUrl = item.notebookUrl ||
        ...
        if (returnPlan.tryDirectFirst && notebookUrl) {
            try {
                if (await navigateDirectlyToNotebookPage(item, notebookUrl, {
        ...
        if (await waitForNotebookReturn(item))
            return;
    ...
    const waitForAnyChatToLoad = async (previousChatId, label = 'chat do caderno', options = {}) => {
        ...
        return waitFor(() => {
            ...
            options.onProgress?.(lastState);
            if (!chatId || chatId === normalizedPreviousChatId)
                return false;
            if (!location.pathname.startsWith('/app/'))
                return false;
            if (turnCount === 0)
                return false;
            if (previousSignature && !changedFromPrevious)
                return false;
            return lastState;
        }, { timeoutMs: FRAME_TIMEOUT_MS, intervalMs: 200, label }).catch((err) => {
            if (lastState) {
                err.message = `${err.message} Ultimo estado: chat=${lastState.chatId || 'nenhum'}, turns=${lastState.turnCount}, mudouDOM=${lastState.changedFromPrevious ? 'sim' : 'nao'}.`;
            }
            throw err;
    ...
    const navigateToKnownChatUrl = async (item, options = {}) => {
        ...
        if (targetChatId) {
            rememberNotebookConversationUrl(item, targetChatId, item.url || `https://gemini.google.com/app/${targetChatId}`);
        }
        const link = document.createElement('a');
    ...
    const navigateToNotebookConversation = async (item, options = {}) => {
        ...
        const previousChatId = currentChatId();
        const previousSignature = conversationDomSignature(document);
        const clickable = element.querySelector('[data-test-id="navigate-to-recent-chat"]') ||
    ...
    const navigateToConversation = async (item, options = {}) => {
        ...
        const preferSidebarNavigation = item.source === 'sidebar' || options.preferSidebarNavigation === true;
    ...
    const getGeminiDomAdapter = () => createGeminiWebDomAdapter({
    ...
    const navigationChatIdMatchesPayload = (navigation, payloadChatId, expectedChatId = '') => {
        ...
        if (!navigation || !normalizedPayloadChatId)
            return false;
        ...
        const hasMatchingChatId = candidateIds.some((value) => stripGeminiConversationPrefix(value || '') === normalizedPayloadChatId);
        if (!hasMatchingChatId)
            return false;
        return (navigation.ok === true ||
            engine.ok === true ||
    ...
    const navigationAllowsSameSignatureExport = (navigation, payloadChatId, expectedChatId = '') => navigation?.acceptedSameSignature === true ||
    ...
    const collectExportForCurrentConversation = async (options = {}) => {
            ...
            onProgress: ({ state: hydrationState, elapsedMs }) => {
                if (state.progress && state.exportSource === 'gui') {
                    updateExportProgress({
                        current: 0,
                        label: `Hidratando conversa... ${hydrationState.turnDomCount || hydrationState.containerCount || 0} turnos vistos`,
        ...
        if (!hydrated.stats.reachedTop || hydrated.stats.timedOut) {
            const reason = hydrated.stats.timedOut
            ...
            throw new Error(`Nao consegui carregar o inicio da conversa com seguranca antes de baixar (${reason}; scroller: ${hydrated.stats.matchedBy}; tentativas: ${hydrated.stats.attempts}; turnos vistos: ${hydrated.stats.turnDomCount || hydrated.stats.turnsAfterHydration || 0}). Recarregue a aba e tente novamente.`);
        }
        ...
        if (expectedChatId && payload.chatId !== expectedChatId) {
            throw new Error(`Download abortado: o navegador abriu o chat ${payload.chatId}, mas a ferramenta local pediu ${expectedChatId}. Nenhum arquivo foi salvo.`);
        }
        if (options.previousSignature &&
            ...
            throw new Error('Download abortado: a URL mudou, mas o conteúdo da conversa ainda parece ser o chat anterior. Tente novamente depois que a página terminar de carregar.');
        }
        return payload;
    };
    const collectExportForConversation = async (item, options = {}) => {
        options.setOperationPhase?.('navigating');
        throwIfOperationAborted(options.abortSignal, 'Exportação cancelada antes de navegar para a conversa.');
        const navigationStartedAt = Date.now();
        const navigation = await openChatWithNavigationEngine(item, {
            ...options,
            ignoreBusy: true,
        });
        throwIfOperationAborted(options.abortSignal, 'Exportação cancelada antes de coletar a conversa.');
    ...
    const downloadBlob = (filename, content) => new Promise((resolve) => {
    ...
    const markdownWithInlineMediaFallback = (payload) => {
        ...
        for (const file of mediaFiles) {
            if (!file?.filename || !file?.contentBase64)
                continue;
            const dataUrl = `data:${file.mimeType || 'application/octet-stream'};base64,${file.contentBase64}`;
        ...
        if (state.bridgeOutputDir) {
            try {
                ...
                if (!state.bridgeSaveFallbackNotified) {
                    ...
                    showToast('Não consegui gravar na pasta escolhida. Vou cair em Downloads; mídias que não baixarem ficam avisadas no Markdown.', 'error');
                }
            }
        }
        ...
        if (isExtensionContext) {
            try {
                ...
                if (!state.browserDownloadFallbackNotified) {
                    ...
                    showToast('A conexão local não respondeu. Vou usar Downloads do navegador. Para salvar direto no vault, reabra o Gemini CLI e clique em Alterar.', 'error');
                }
            }
        }
    ...
    const exportNow = async () => {
        ...
        if (!chatId) {
            const message = 'Abra uma conversa específica antes de baixar (URL precisa conter /app/<id>).';
        ...
        if (isExtensionContext) {
            try {
                ...
                showToast('Não consegui iniciar o export privado pela ferramenta local. Reabra o Gemini CLI e tente de novo.', 'error');
            }
            return;
        }
        if (scrapeTurns(document).length === 0) {
            const message = 'Nenhum turno detectado. Veja o console e use window.__geminiMdExportDebug.snapshot() para inspecionar o DOM.';
            reportFailure(message);
            alert(message);
            return;
        }
        try {
            ...
            showToast(`Pronto! ${payload.turns.length} turnos salvos em ${payload.filename}.`, 'success');
        }
        catch (err) {
            warn('Falha ao baixar conversa atual.', err);
            await finishExportProgress();
            showToast('Não consegui baixar essa conversa. Abra o console (F12) para ver o motivo.', 'error');
        }
    };
    const renderListEndState = () => {
        ...
        if (!endEl)
            return;
        endEl.className = `gm-list-end ${state.isLoadingMore
            ? 'is-loading'
            : state.reachedSidebarEnd
                ? 'is-end'
    ...
    const filteredConversationsForModal = () => state.conversations.filter((item) => !state.filterQuery ||
    ...
    const renderConversationItemHtml = (item) => {
        ...
        const currentBadge = item.current
            ...
            : '';
        const notebookBadge = item.source === 'notebook'
            ? '<span class="gm-badge">Caderno</span>'
            : '';
        const subtitle = item.subtitle && item.subtitle !== item.title
    ...
    const modalVirtualListKey = (filtered) => `${state.filterQuery}|${state.selectedChatIds.size}|${filtered.length}|${filtered[0]?.id || ''}|${filtered.at(-1)?.id || ''}`;
    const scheduleVirtualListRender = () => {
        if (!state.modalVirtual.active || state.modalVirtual.scheduled)
            return;
    ...
    const modalListMaxScrollTop = (target) => computeModalVirtualScrollRange(modalListWheelMetrics(target));
    const handleModalListScrollPosition = (target) => {
        scheduleVirtualListRender();
        if (modalListMaxScrollTop(target) - target.scrollTop < 120) {
            loadMoreConversations(isNotebookPage() ? NOTEBOOK_LOAD_MORE_ATTEMPTS : SIDEBAR_LOAD_MORE_ATTEMPTS);
        }
    };
    ...
    const handleModalPanelWheel = (event) => {
        ...
        if (target instanceof Element) {
            if (target.closest(`#${MODAL_LIST_ID}`))
                return;
    ...
    const renderConversationList = ({ fromVirtualScroll = false } = {}) => {
        ...
        const wasNearBottom = maxScrollTopBeforeRender > 0 && maxScrollTopBeforeRender - container.scrollTop < 48;
        const finishRender = () => {
            renderListEndState();
      ...
      `);
        ...
        if (shouldVirtualize) {
            ...
            const start = Math.max(0, Math.floor(prevScrollTop / MODAL_VIRTUAL_ITEM_HEIGHT) - MODAL_VIRTUAL_BUFFER);
            const visibleCount = Math.ceil(viewportHeight / MODAL_VIRTUAL_ITEM_HEIGHT) + MODAL_VIRTUAL_BUFFER * 2;
            const end = Math.min(filtered.length, start + visibleCount);
            const unchangedWindow = fromVirtualScroll &&
            ...
            <div class="gm-virtual-spacer" style="height:${Math.max(0, (filtered.length - end) * MODAL_VIRTUAL_ITEM_HEIGHT)}px"></div>
          `);
                if (prevScrollTop > 0)
        ...
        setHtml(container, filtered.map(renderConversationItemHtml).join(''));
        finishRender();
    };
    ...
    const UI_TECHNICAL_COPY_RE = /\b(MCP|bridge|claim|job|payload|phase|reexport(?:ando|acao|ação)?)\b/i;
    ...
    const safeProgressItemLabel = (progress) => {
        ...
        if (label && !UI_TECHNICAL_COPY_RE.test(label))
            return label;
    ...
    const humanProgressLabelFor = (progress) => {
        ...
        if (status === 'cancel_requested')
            return 'Cancelando com segurança...';
        if (status === 'completed')
            return 'Concluído';
        if (status === 'completed_with_errors')
        ...
        if (status === 'failed')
            return 'Falhou';
        if (phase === 'loading-history')
            return 'Lendo histórico do Gemini...';
        if (phase === 'scanning-vault')
            return 'Comparando com o vault...';
        if (phase === 'writing-report')
            return 'Salvando relatório...';
        if (phase === 'exporting') {
        ...
        if (fallback && !UI_TECHNICAL_COPY_RE.test(fallback))
            return fallback;
        return 'Preparando...';
    };
    const humanCountFor = (progress) => {
        if (progress?.sourceKind && progress?.countLabel)
    ...
    const buildBrowserProgressView = (progress, sourceKind = 'gui-export') => {
        ...
        const currentItem = raw.title || raw.currentChatId || raw.chatId
            ? {
    ...
    const startProgressCreep = () => {
        ...
        state.progressCreepTimer = setInterval(() => {
            ...
            if (barEl)
                barEl.style.width = `${next_display}%`;
    ...
    const beginExportProgress = async ({ total, label, phase = 'preparando', kind = null, workflow = null, status = 'running', }) => {
    ...
    const updateExportProgress = (patch = {}) => {
        ...
        const normalizedPatch = {
            ...patch,
            errorCount: state.exportSource === 'mcp'
        ...
        state.progress.displayPercent = normalizeProgressDisplayPercent(previousProgress, state.progress, previousProgress.displayPercent ?? state.progress.displayPercent ?? 0);
    ...
    const clearActiveTabOperationForTerminalMcpJob = (jobId) => {
        const active = state.activeTabOperation;
        if (!active || !jobId || !active.jobId || active.jobId !== jobId)
            return false;
        state.completedTabOperations += 1;
        state.activeTabOperation = null;
    ...
    const mcpTerminalSignaturePart = (value) => value === null || value === undefined ? '' : String(value).replace(/[|\r\n]/g, ' ');
    const buildMcpTerminalProgressSignature = (jobProgress, fallbackJobId = null) => {
        ...
        if (!TERMINAL_MCP_STATUSES.has(status))
            return null;
    ...
    const clearStaleMcpProgressIfNeeded = () => {
        ...
        const now = Date.now();
        const staleMs = status === 'cancel_requested'
            ? now - (state.mcpProgressCancelSinceAt || state.mcpProgressLastSeenAt || now)
            : now - (state.mcpProgressLastSeenAt || now);
        const limitMs = status === 'cancel_requested' ? MCP_PROGRESS_CANCEL_STALE_MS : MCP_PROGRESS_STALE_GRACE_MS;
        if (staleMs < limitMs)
            return false;
    ...
    const startMcpProgressWatchdog = () => {
        if (state.mcpProgressWatchdogTimer)
            return;
        state.mcpProgressWatchdogTimer = setInterval(clearStaleMcpProgressIfNeeded, MCP_PROGRESS_WATCHDOG_MS);
    };
    ...
    const saveMcpProgressSnapshot = (jobProgress) => {
        ...
        if (!snapshot)
            return;
        try {
            pageWindow.sessionStorage?.setItem(MCP_PROGRESS_SESSION_STORAGE_KEY, JSON.stringify(snapshot));
        }
        catch {
    ...
    const loadMcpProgressSnapshot = () => {
        try {
            ...
            if (!snapshot || snapshot.source !== 'mcp')
                return null;
    ...
    const handleMcpJobProgressBroadcast = (jobProgress) => {
        ...
        if (!jobProgress) {
            ...
            if (state.mcpProgressActive) {
                if (clearStaleMcpProgressIfNeeded())
                    return;
        ...
        if (jobProgress.source && jobProgress.source !== 'mcp')
            return;
        const isTerminalMcpProgress = Boolean(jobProgress.status && TERMINAL_MCP_STATUSES.has(jobProgress.status));
    ...
    const runBatchExport = async (items, { session = null, originalItem = null, originalWasNotebook = false, originalNotebookReturnItem = null, resume = false, } = {}) => {
        ...
        let activeSession = normalizeBatchExportSession(session) ||
        ...
        if (failures.size === 0) {
            ...
            showToast(`Pronto! ${n} ${n === 1 ? 'conversa baixada' : 'conversas baixadas'} com sucesso.`, 'success');
        }
        else {
            const failed = failures.size;
            const total = activeSession.items.length;
            showToast(`Terminou com ${failed} de ${total} ${failed === 1 ? 'falha' : 'falhas'}. Abra o console (F12) para ver os detalhes.`, 'error');
        }
    };
    const updateModalState = () => {
        ...
        const canExport = state.selectedChatIds.size > 0 &&
            !state.isExporting;
        updateProgressDock();
        ...
        [folderBtn, refreshBtn, selectAllBtn, clearBtn, closeBtn, currentBtn].forEach((btn) => {
            if (btn)
                btn.disabled = state.isExporting;
        });
        if (searchEl)
            searchEl.disabled = state.isExporting;
    };
    const ensureModal = () => {
    ...
    `);
        ...
        modal.addEventListener('change', (event) => {
            ...
            if (!(target instanceof HTMLInputElement))
                return;
            if (target.type !== 'checkbox' || !target.dataset.chatId)
                return;
            if (target.checked) {
        ...
        modal.addEventListener('click', async (event) => {
            ...
            if (action === 'refresh-list') {
                ...
                state.reachedSidebarEnd = false;
                state.listLoadStatus = 'idle';
                const loaded = await loadMoreConversations(isNotebookPage() ? NOTEBOOK_LOAD_MORE_ATTEMPTS : SIDEBAR_LOAD_MORE_ATTEMPTS);
                if (!loaded) {
                    refreshConversationState();
                    updateModalState();
                    showToast(state.reachedSidebarEnd ? listEndStatusText() : listInconclusiveStatusText(), 'info');
                }
                return;
            }
            ...
            if (action === 'pick-dir') {
                if (isExtensionContext) {
                    ...
                    if (result.status === 'picked') {
                        ...
                        showToast('Não consegui abrir o seletor. Reabra o Gemini CLI ou rode o instalador de novo para restaurar a ferramenta local.', 'error');
                        updateModalState();
                    }
                    return;
                }
                if (!supportsDirectoryPicker()) {
                    showToast('Este navegador não suporta escolher pasta. Use Chrome/Edge ou a ferramenta local.', 'error');
                    return;
                }
                try {
                    ...
                    if (errorText.includes('system files') ||
                        errorText.includes('sensitive') ||
                        errorText.includes('restricted')) {
                        showToast('O navegador bloqueou essa pasta. Escolha uma subpasta comum, por exemplo Downloads/Gemini Exports.', 'error');
                    }
                    else {
                        showToast('Não consegui salvar a pasta escolhida. Tente outra pasta ou reinicie a ferramenta local.', 'error');
                    }
                }
                return;
            }
            ...
            if (action === 'run-export') {
                const selected = state.conversations.filter((item) => state.selectedChatIds.has(item.id));
                if (selected.length === 0) {
                    showToast('Marque pelo menos uma conversa na lista antes de baixar.', 'info');
                    return;
                }
                if (selected.some((item) => item.exportable === false)) {
                    showToast('Algumas conversas do caderno não expõem ID — não consigo baixar em lote. Abra uma por uma e use "Baixar atual".', 'error');
                    return;
                }
                try {
    ...
    const safeOpenExportModal = async () => {
        try {
            ...
            showToast('Não consegui abrir o modal de download. Abra o console (F12) para ver o motivo.', 'error');
        }
    };
    const refreshExtensionContext = async ({ force = false } = {}) => {
        const missingContext = bridgeState.tabId === null ||
        ...
        try {
            const ping = await extensionSendMessageWithRetry({ type: 'gemini-md-export/ping' }, { timeoutMs: 2500, attempts: 2, retryDelayMs: 160 });
            ...
            if (extensionContext?.tabId !== undefined)
                bridgeState.tabId = extensionContext.tabId;
            if (extensionContext?.windowId !== undefined)
                bridgeState.windowId = extensionContext.windowId;
            if (extensionContext?.isActiveTab !== undefined) {
                bridgeState.isActiveTab = extensionContext.isActiveTab;
            }
            if (extensionContext?.version)
                bridgeState.extensionVersion = extensionContext.version;
            if (extensionContext?.protocolVersion !== undefined) {
                bridgeState.protocolVersion = extensionContext.protocolVersion;
            }
            if (extensionContext?.buildStamp)
                bridgeState.buildStamp = extensionContext.buildStamp;
        }
        catch (err) {
            bridgeState.lastExtensionPingAt = Date.now();
    ...
    const sendBridgeHeartbeat = async () => {
        if (!bridgeState.started || bridgeState.heartbeatInFlight)
            return;
    ...
    const getChatBridgeClient = () => {
        ...
        chatBridgeClient = createBrowserBridgeClient({
            ...
            executeCommand: (command) => runWithTabOperationBackpressure(command, (operationContext) => executeBridgeCommand(command, operationContext)),
    ...
    const resumePendingBridgeCommand = async () => {
        if (pendingBridgeCommandResumeInFlight || !bridgeState.started)
            return;
    ...
    const scheduleBridgeEventsReconnect = () => {
        if (!bridgeState.started || bridgeState.eventsReconnectTimer)
            return;
    ...
    const buildBridgeEventsSearchParams = () => {
        ...
        if (claim?.claimId)
            params.set('claimId', String(claim.claimId));
        if (claim?.sessionId)
            params.set('claimSessionId', String(claim.sessionId));
    ...
    const pollBridgeCommands = async (enabled = true, { force = false } = {}) => {
        if (!enabled)
            return;
        if (!bridgeState.started || bridgeState.polling || (bridgeState.eventsConnected && !force))
            return;
        ...
        while (bridgeState.started && (!bridgeState.eventsConnected || force)) {
            try {
                ...
                const response = await bridgeRequest(`/bridge/command?clientId=${encodeURIComponent(bridgeState.clientId)}`, { timeoutMs: BRIDGE_POLL_TIMEOUT_MS });
                ...
                errorBackoffMs = Math.min(BRIDGE_EVENTS_MAX_BACKOFF_MS, bridgeBackoffWithJitter(errorBackoffMs * 2, BRIDGE_POLL_ERROR_BACKOFF_MS, BRIDGE_EVENTS_MAX_BACKOFF_MS));
            }
        }
    ...
    const scheduleExtensionBridgeInstallRetry = (reason = 'bridge-start-retry') => {
        if (bridgeInstallRetryTimer || bridgeState.started || isTabIgnored())
            return;
        const delayMs = bridgeInstallRetryDelayMs;
        bridgeInstallRetryDelayMs = Math.min(BRIDGE_INSTALL_RETRY_MAX_MS, Math.round(bridgeInstallRetryDelayMs * 1.7));
        bridgeInstallRetryTimer = setTimeout(() => {
            bridgeInstallRetryTimer = 0;
            if (bridgeState.started || isTabIgnored())
                return;
            installExtensionBridge()
                ...
                .finally(() => {
                if (!bridgeState.started)
                    scheduleExtensionBridgeInstallRetry('bridge-start-retry');
            });
        }, delayMs);
    ...
    const installExtensionBridge = async () => {
        if (!isExtensionContext || bridgeState.started)
            return;
        if (isTabIgnored()) {
    ...
    const installTopBarButtonFallbackStateHandlers = (btn) => {
        if (btn.dataset.gmTopBarStateHandlers === '1')
            return;
        ...
        const setButtonState = (state = 'normal') => {
            ...
            const backgrounds = {
                normal: 'transparent',
                hover: nativeStyleVar('--gmn-topbar-state-hover', 'var(--gem-sys-color--surface-container-highest, rgba(232, 234, 237, 0.08))'),
                focus: nativeStyleVar('--gmn-topbar-state-focus', 'var(--gem-sys-color--surface-container-highest, rgba(232, 234, 237, 0.10))'),
                pressed: nativeStyleVar('--gmn-topbar-state-pressed', 'var(--gem-sys-color--surface-container-highest, rgba(232, 234, 237, 0.14))'),
            };
    ...
    const positionTopBarTooltip = (tooltip, btn) => {
        ...
        const viewportWidth = pageWindow.innerWidth || document.documentElement?.clientWidth || 0;
        const viewportHeight = pageWindow.innerHeight || document.documentElement?.clientHeight || 0;
    ...
    const installTopBarTooltipHandlers = (btn) => {
        if (btn.dataset.gmTopBarTooltipHandlers === '1')
            return;
    ...
    const buildMenuPalette = () => buildMenuHostPalette({ documentRef: document, isDark: isDarkTheme() });
    ...
    const positionTopBarMenu = (menu, btn) => {
        ...
        const top = Math.min(Math.max(8, Math.round(rect.bottom + 8)), Math.max(8, viewportHeight - 8));
    ...
    const installMenuItemStateHandlers = (el) => {
        if (el.dataset.gmMenuStateHandlers === '1')
            return;
        ...
        el.addEventListener('mouseup', () => setMenuItemVisualState(el, pointerInside ? 'hover' : 'normal'));
    ...
    const styleMenuItem = (el, options = {}) => {
        const minHeight = options.minHeight || nativeStyleVar('--gmn-menu-item-min-height', '56px');
    ...
    const renderIgnoreMenuItem = (item) => {
        ...
        item.setAttribute('aria-label', ignored
    ...
    const openTopBarMenu = (btn) => {
        ...
        const referenceItem = referenceMenu?.querySelector('button.mat-mdc-menu-item, [role="menuitem"]') || null;
        ...
        Object.assign(menu.style, {
            ...
            padding: '0',
            overflow: 'hidden',
            background: 'var(--gem-sys-color--surface-container, var(--mat-sys-surface-container, var(--gm-menu-bg)))',
            color: 'var(--mat-sys-on-surface, var(--gm-menu-text))',
            borderRadius: 'var(--gem-sys-shape--corner-xl, var(--gmn-menu-radius, 20px))',
            boxShadow: nativeStyleVar('--gmn-menu-shadow', 'rgba(0, 0, 0, 0.28) 0px 0px 20px 0px'),
        ...
        Object.assign(divider.style, {
            ...
            background: 'var(--gem-sys-color--outline-variant, var(--mat-sys-outline-variant, var(--gm-menu-divider)))',
            border: 'none',
        });
        ...
        ignoreItem.addEventListener('click', () => {
            ...
            try {
                showToast(next
                    ...
                    : 'Aba reativada. A conexão local voltou.', 'info');
            }
            catch {
        ...
        menuRepositionHandler = () => {
            ...
            if (live && btnLive)
                positionTopBarMenu(live, btnLive);
        };
    ...
    const findNativeIconButtonReference = (topBar = null) => {
        ...
        const collectCandidate = (el) => {
            if (!(el instanceof HTMLElement) || seen.has(el))
                return;
            seen.add(el);
            if (el.id === BUTTON_ID || el.closest?.(`#${BUTTON_SLOT_ID}`))
                return;
            const rect = el.getBoundingClientRect();
            if (rect.width <= 0 || rect.height <= 0)
                return;
        ...
        if (topBar instanceof Element) {
            const scopedRoot = topBar.querySelector('.right-section') ||
                topBar.querySelector('.top-bar-actions') ||
                topBar;
            for (const el of scopedRoot.querySelectorAll('button.mat-mdc-icon-button, button[mat-icon-button], conversation-actions-icon button, temp-chat-button button, button[aria-haspopup="menu"], button')) {
                collectCandidate(el);
            }
        }
        for (const selector of selectors) {
    ...
    const swapClonedButtonIcon = (clone) => {
        const iconHosts = clone.querySelectorAll('mat-icon, gem-icon, .material-symbols-outlined, .gds-icon, svg');
        if (iconHosts.length) {
            const firstIconHost = iconHosts[0];
            if (firstIconHost instanceof SVGElement) {
                const svg = createDownloadIconSvg();
                if (svg)
                    firstIconHost.replaceWith(svg);
    ...
    const findTopBarPlacementAnchor = (topBar) => {
        const controls = Array.from(topBar.querySelectorAll('button,[role="button"],a[role="button"]')).filter(isVisibleControl);
    ...
    const resolveTopBarPlacement = (topBar, anchor) => {
        if (!anchor)
            return { host: topBar, before: null };
    ...
    const placeInTopBar = (btn, placementTarget) => {
        ...
        const alreadyInPlace = slot.parentElement === placement.host &&
    ...
    const injectButton = () => {
        ...
        if (!found) {
            ...
            if (stuck) {
                ...
                warn(`top-bar não encontrado após ${Math.round(NOT_FOUND_GRACE_MS / 1000)}s numa URL de conversa. Diagnóstico:`, diagnostics);
            }
            return;
        }
        ...
        if (existing) {
            ...
            if (!existing.querySelector('svg[data-role="gm-md-export-download-icon"]') ||
    ...
    const installDebugApi = () => {
        pageWindow[DEBUG_GLOBAL] = {
            ...
            loadMoreConversations: () => {
                ...
                state.reachedSidebarEnd = false;
                state.listLoadStatus = 'idle';
                return loadMoreConversations(isNotebookPage() ? NOTEBOOK_LOAD_MORE_ATTEMPTS : SIDEBAR_LOAD_MORE_ATTEMPTS);
            },
            bridgeStatus: () => ({
                started: bridgeState.started,
                clientId: bridgeState.clientId,
            ...
            collectCurrentConversationForDebug: (options = {}) => collectExportForCurrentConversation(options),
            conversationDomSignature: () => conversationDomSignature(document),
            waitForChatToLoadForDebug: (targetChatId, options = {}) => waitForChatToLoad(targetChatId, options),
            findTopBar: () => {
                const res = findTopBar();
                if (!res)
                    return { matchedBy: null, target: null };
                return { matchedBy: res.matchedBy, target: res.target };
            },
            scrapeTurns: () => scrapeTurns(document),
            markdown: () => buildDocument({
                meta: {
    ...
    const resumePendingBatchExport = async () => {
        ...
        showToast(`Retomando download interrompido — conversa ${session.nextIndex + 1} de ${session.items.length}...`, 'info');
        try {
            ...
            showToast('Não consegui retomar o download pendente. Abra o console (F12) para ver o motivo.', 'error');
        }
    };
    // --- hotkey -----------------------------------------------------------
    const isEditableTarget = (target) => !!target &&
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.key.toLowerCase() === HOTKEY.key &&
    ...
    const bootstrap = () => {
        ...
        const observer = new MutationObserver(() => scheduleDomWork('body-mutation', { topBar: true }));
        ...
        log(`userscript carregado (v0.8.60 build 20260604-0112)`);
