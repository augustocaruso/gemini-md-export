export const browserAuthorityLeaseToken = (lease) => ({
    leaseId: lease.leaseId,
    operationId: lease.operationId,
    deadlineAtMs: lease.budget.deadlineAtMs,
});
export const assertLeasedBrowserEffect = ({ effect, token, nowMs = Date.now(), }) => {
    if (!token) {
    ...
    if (!effect.leaseId || effect.leaseId !== token.leaseId) {
        throw Object.assign(new Error('Este comando de navegador nao pertence a autorizacao desta operacao.'), { code: 'browser_authority_lease_mismatch' });
    }
