...
export const readCurrentBridgeVersion = ({ root, bridgeVersion, }) => readJsonFileSafe(resolve(root, 'bridge-version.json'), bridgeVersion) || bridgeVersion;
export const readCurrentPackageVersion = ({ root, packageVersion, serverVersion, }) => readJsonFileSafe(resolve(root, 'package.json'), { version: packageVersion })
    ?.version || serverVersion;
export const detectExpectedBrowserBuildStamp = ({ root, bridgeVersion, env = process.env, homeDir = homedir(), }) => {
    if (env.GEMINI_MCP_EXPECTED_BUILD_STAMP)
        return env.GEMINI_MCP_EXPECTED_BUILD_STAMP;
    ...
    for (const candidate of candidates) {
        try {
            ...
            const match = source.match(/\bbuildStamp:\s*['"](\d{8}-\d{4})['"]/) ||
...
export const createExpectedChromeExtensionInfo = (options) => {
    const snapshot = () => ({
        extensionVersion: String(readCurrentBridgeVersion(options).extensionVersion || '') ||
            readCurrentPackageVersion(options),
        protocolVersion: options.protocolVersion,
        buildStamp: detectExpectedBrowserBuildStamp(options),
    });
    const info = {};
    for (const key of ['extensionVersion', 'protocolVersion', 'buildStamp']) {
