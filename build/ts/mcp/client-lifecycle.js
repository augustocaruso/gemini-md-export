const CLAIMABLE_GEMINI_TAB = Symbol('ClaimableGeminiTab');
const CLAIMED_READY_GEMINI_TAB = Symbol('ClaimedReadyGeminiTab');
...
const MANUAL_RELOAD_CODES = new Set([
...
const pageUrl = (client) => {
    const url = client.page?.url;
    return typeof url === 'string' && url.length > 0 ? url : null;
};
const pageOrigin = (client) => {
...
const result = (state, code, client, claim) => ({
    ok: code === null,
    state,
    code,
...
const hasExpired = (claim, now) => typeof claim.expiresAtMs === 'number' && claim.expiresAtMs <= now;
const claimTargetsClient = (claim, client) => {
    if (claim.clientId && claim.clientId === client.clientId)
        return true;
    return tabIdsMatch(claim.tabId, client.tabId);
};
const findClaimForClient = (client, options) => {
    const now = Number(options.now ?? Date.now());
    const claims = options.claims || [];
    const claimId = normalizeString(options.claimId);
    const sessionId = normalizeString(options.sessionId);
    return (claims.find((claim) => {
        if (hasExpired(claim, now))
            return false;
        if (claimId && claim.claimId !== claimId)
            return false;
        if (!claimTargetsClient(claim, client))
            return false;
        if (sessionId && claim.sessionId && claim.sessionId !== sessionId)
            return false;
        return true;
    }) || null);
};
const hasConflictingClaim = (client, options) => {
    const now = Number(options.now ?? Date.now());
    const sessionId = normalizeString(options.sessionId);
    const claimId = normalizeString(options.claimId);
...
const pageHasHydratedGeminiContext = (page) => {
    const pathname = pagePathname(page);
    if (normalizeString(page.chatId))
        return true;
    if (normalizeString(page.notebookId))
        return true;
    if (/^\/app\/[a-f0-9]{12,}/i.test(pathname))
        return true;
    ...
    if (Number(page.sidebarConversationCount || 0) > 0)
        return true;
    if (Number(page.notebookCacheCount || 0) > 0)
        return true;
    if (page.sidebarOpen === true)
        return true;
    return false;
};
const pageIsGeminiAppHome = (page) => {
    const pathname = pagePathname(page);
    return pathname === '/app' || pathname === '/app/';
};
const pageIsGeminiAppRoute = (page) => {
    const pathname = pagePathname(page);
    return pathname === '/app' || pathname === '/app/' || pathname.startsWith('/app/');
};
const pageHasRecentExportContext = (page) => {
    if (!pageIsGeminiAppRoute(page))
        return false;
    if (pageHasHydratedGeminiContext(page))
        return true;
    if (pageIsGeminiAppHome(page))
        return true;
    return false;
};
const pageHasCurrentChatContext = (page) => {
    const pathname = pagePathname(page);
    if (normalizeString(page.chatId))
        return true;
    if (/^\/app\/[a-f0-9]{12,}/i.test(pathname))
...
const pageBlockerCode = (page) => {
    ...
    if (code === 'google_page_blocked')
        return 'google_page_blocked';
    ...
    if (kind === 'google_login')
        return 'google_login_required';
    if (page?.blocker?.terminal === true)
        return 'google_page_blocked';
    return null;
};
const activeTabOperation = (client) => client.tabOperationInProgress === true ||
    Boolean(client.metrics?.tabOperation?.active) ||
    Boolean(client.summary?.metrics?.tabOperation?.active);
const effectiveIsActiveTab = (client) => client.isActiveTab === true || client.page?.isActiveTab === true;
const runtimeSignalAt = (client) => {
...
export const getGeminiClientLifecycle = (client, options) => {
    if (!client?.clientId) {
    ...
    if (tabId === null)
        return result('page_unready', 'missing_tab_id', client);
...
export const toClaimableGeminiTab = (client, options) => {
    const lifecycle = getGeminiClientLifecycle(client, options);
    if (lifecycle.state !== 'claimable' || !client?.page?.url)
        return null;
    return {
        ...client,
...
export const toClaimedReadyGeminiTab = (client, options) => {
    const lifecycle = getGeminiClientLifecycle(client, {
        ...options,
        requireClaimed: true,
        requireCommandReady: true,
    });
    if (lifecycle.state !== 'claimed_ready' || !client?.page?.url || !lifecycle.claim)
        return null;
    return {
        ...client,
        tabId: normalizeNumber(client.tabId) ?? 0,
...
export const assertClaimableGeminiTab = (client, options) => {
...
export const assertClaimedReadyGeminiTab = (client, options) => {
...
export const getClaimableGeminiTabs = (clients = [], options) => clients.flatMap((client) => {
    const claimable = toClaimableGeminiTab(client, options);
    return claimable ? [claimable] : [];
});
export const classifyGeminiClientLifecycle = (clients = [], options) => clients.map((client) => ({
    client,
    lifecycle: getGeminiClientLifecycle(client, options),
}));
export const explainGeminiClientLifecycleRejection = (client, options) => {
    const lifecycle = getGeminiClientLifecycle(client, options);
    if (lifecycle.ok)
        return null;
    return {
