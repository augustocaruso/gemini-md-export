...
export const outputDirForDirectReexportItem = (item, fallbackOutputDir) => stringOrNull(item?.outputDir) || stringOrNull(fallbackOutputDir) || '';
const collectRawDirectReexportItems = (args = {}) => {
    const rawItems = [];
    if (Array.isArray(args.chatIds)) {
        for (const chatId of args.chatIds)
            rawItems.push({ chatId });
    }
    ...
    if (args.chatId)
        rawItems.push({ chatId: args.chatId, title: args.title });
    return rawItems;
};
...
export const normalizeDirectReexportSelection = (args = {}, options) => {
    ...
    if (expectedCount !== null) {
        ...
        if (normalized.length !== expectedCount) {
            const err = new Error(`A seleção recebida tem ${normalized.length} chatId(s) único(s), mas expectedCount=${expectedCount}.`);
