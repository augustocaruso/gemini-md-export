...
export const buildConversationExportSuccess = ({ index, conversation, result, }) => ({
    index,
...
export const recordConversationExportSuccess = ({ job, successes, itemMetric, success, result }, deps) => {
    ...
    deps.recordJobCounter(job, 'savedBytes', (result.bytes || 0) + (result.metrics?.counters?.savedMediaBytes || 0));
...
export const buildConversationExportFailure = ({ index, conversation, err, }) => ({
    index,
...
export const recordConversationExportFailure = ({ job, failures, itemMetric, failure, err }, deps) => {
...
const prepareDeferredDateImportSaves = async ({ job, client, successes, failures, deferredSaves, args, persist, broadcast }, deps) => {
    ...
    const batchEvidence = await deps.measureJobTiming(job, 'resolveDateImportBatchMs', async () => deps.buildExportDateImportBatchEvidenceForPayloads(deferredSaves.map(({ collected }) => ({
...
export const saveDeferredDateImportExports = async (input, deps) => {
    let { client } = input;
    const shouldSave = await prepareDeferredDateImportSaves(input, deps);
    if (!shouldSave)
        return client;
    try {
        for (const { index, conversation, itemMetric, collected, } of input.deferredSaves) {
            input.job.current = {
                index,
                title: conversation.title || null,
                chatId: conversation.chatId || conversation.id || null,
...
export const loadDateImportForExportJob = async ({ job, client, args, successes, failures, persist, broadcast }, deps) => {
    ...
    persist(job, client, successes, failures);
    broadcast(job, client);
    const context = await deps.measureJobTiming(job, 'loadDateImportSourceMs', async () => deps.createExportDateImportContextForArgs(args));
