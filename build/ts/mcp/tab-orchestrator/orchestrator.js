...
import { allocateTabForPurpose, initialTabPoolState, reduceTabLifecycle, } from './tab-lifecycle-fsm.js';
const blocker = (code, message, severity = 'warning') => ({ code, message, severity });
const observeEvidence = (desiredEpochId, nowMs, evidence, clients) => {
...
const evidenceMatchesTab = (item, tab) => {
    if (item.tabId !== null)
        return tab.tabId === item.tabId;
    if (item.clientId !== null)
        return tab.clientId === item.clientId;
    return false;
};
const hydrateObservedLease = (state, item, leaseClaimId) => ({
    ...state,
...
const modeAllowsRuntimeRecovery = (mode) => mode === 'activity_scan' || mode === 'interactive';
const recoveryForEvidence = (desiredEpochId, nowMs, item) => {
...
export const planTabOrchestration = (request) => {
    const desiredEpochId = runtimeEpochId(request.expected);
    const evidence = request.clients.map((client) => classifyRuntimeEvidence({
        client,
    ...
    if (allocation.status === 'ambiguous') {
        return {
            ready: false,
            blocker: blocker('ambiguous_tabs', 'Mais de uma aba atende ao pedido; selecione ou reivindique uma aba explicitamente.'),
            effects: allocation.effects,
            evidence,
            state: allocation.state,
            diagnostics,
    ...
    const weakMatchingEvidence = matchingPageEvidence.find((item) => item.epochId === desiredEpochId && item.strength === 'weak');
    if (weakMatchingEvidence) {
        return {
            ready: false,
            blocker: blocker('command_channel_not_ready', 'A aba respondeu no runtime esperado, mas ainda nao tem canal de comando forte.'),
            effects: [],
            evidence,
            state: allocation.state,
            diagnostics,
    ...
    if (rejectedEvidence) {
        ...
        return {
            ready: false,
            blocker: blocker('runtime_epoch_not_ready', 'A aba observada pertence a outro runtime da extensao ou esta stale.', 'error'),
