...
export const evaluateConversationOperationWatchdog = ({ now, lastProgressAt, noProgressMs, cancelRequested = false, }) => {
...
export const evaluateConversationNoProgressBudgetFsm = ({ requestedMs = 0, browserNavigationTimeoutMs = 0, recoveryGapMs = 0, maxMs = Number.POSITIVE_INFINITY, } = {}) => {
    ...
    const safeMaxMs = isFiniteNonNegativeMs(maxMs) && maxMs > 0 ? Math.floor(maxMs) : Number.POSITIVE_INFINITY;
    ...
    return {
        ...
        minimumMs,
        noProgressMs,
        reason: noProgressMs > safeRequestedMs ? 'raised_above_browser_navigation_timeout' : 'requested_ok',
    };
};
