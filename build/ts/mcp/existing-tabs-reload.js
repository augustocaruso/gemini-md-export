...
const numberOrStringValue = (value) => {
...
const extensionStatusValue = (status, key) => {
    if (!isRecord(status))
        return null;
...
const expectedValue = (expected, key) => (isRecord(expected) ? numberOrStringValue(expected[key]) : null);
export const evaluateExistingTabsRuntimeRefreshFsm = ({ allowReload = false, expected = null, extensionStatus = null, } = {}) => {
...
const nativeReloadTabIds = (result) => {
    if (!isRecord(result) || !Array.isArray(result.reloadedTabIds))
        return [];
...
const candidatePage = (candidate) => (isRecord(candidate.page) ? candidate.page : {});
const candidateIsActive = (candidate) => candidate.isActiveTab === true || candidatePage(candidate).isActiveTab === true;
const candidateClientId = (candidate) => stringValue(candidate.clientId) || undefined;
const candidateUrl = (candidate) => stringValue(candidatePage(candidate).url);
const candidateKind = (candidate) => stringValue(candidatePage(candidate).kind);
const candidateLooksLikeGeminiApp = (candidate) => {
    const kind = candidateKind(candidate);
    if (kind === 'activity')
        return false;
    const url = candidateUrl(candidate);
    if (!url)
        return kind === 'chat' || kind === 'gemini';
    try {
...
const candidateContextScore = (candidate) => {
    const page = candidatePage(candidate);
    let score = 0;
    if (candidate.commandReady === true)
        score += 100;
    if (stringValue(page.chatId))
        score += 30;
    if (stringValue(page.notebookId))
        score += 20;
    if (Number(page.turnCount || 0) > 0)
        score += 12;
    if (Number(page.listedConversationCount || 0) > 0)
        score += 10;
    if (Number(page.bridgeConversationCount || 0) > 0)
        score += 10;
    if (Number(page.sidebarConversationCount || 0) > 0)
        score += 10;
    if (page.sidebarOpen === true)
        score += 4;
    return score;
};
const candidateLastSignal = (candidate) => Math.max(Number(candidate.lastHeartbeatAt || 0), Number(candidate.lastSnapshotAt || 0), Number(candidate.lastSeenAt || 0));
export const evaluateBrowserReadyInactiveTabActivationFsm = ({ allowActivation = false, ready = false, claimableClientCount = 0, selectableClients = [], } = {}) => {
    ...
    const candidates = selectableClients
        ...
        .sort((left, right) => candidateContextScore(right.candidate) - candidateContextScore(left.candidate) ||
...
export const maybeActivateBrowserReadyInactiveTab = async (input = {}, deps) => {
    const args = input.args || {};
    const decision = evaluateBrowserReadyInactiveTabActivationFsm({
        ...input,
        allowActivation: input.allowActivation ??
    ...
    const activationCandidate = (input.selectableClients || []).find((client) => positiveIntegerValue(client.tabId) === decision.tabId) || null;
    try {
        const activation = await deps.activateBrowserTabById(decision.tabId, {
            ...args,
...
const attachedContentScriptSelfHealFailed = (result) => isRecord(result) &&
    isRecord(result.contentScriptSelfHeal) &&
    result.contentScriptSelfHeal.ok === false;
const attachedContentScriptSelfHealCompleted = (result) => isRecord(result) &&
    isRecord(result.contentScriptSelfHeal) &&
    result.contentScriptSelfHeal.ok === true;
const reloadReadyClients = (clients, predicate) => {
    const items = [...clients];
    return predicate ? items.filter(predicate) : items;
};
export const evaluateExistingTabsPostReloadRecoveryFsm = ({ allowReload = false, connectedClientCount = 0, nativeReload = null, } = {}) => {
    ...
    const shouldSelfHeal = nativeReload.ok === true || (code !== null && RECOVERABLE_NATIVE_RELOAD_CODES.has(code));
    if (!shouldSelfHeal) {
        return {
            state: 'blocked',
...
export const reloadSideEffectExplicitlyAllowed = (args = {}) => args.allowReload === true ||
    args.explicit === true ||
    args.intent === 'tab_management' ||
    args.diagnostic === true;
export const existingTabsReloadHttpFallbackArgs = (args = {}) => ({
    ...args,
...
export const shouldUseDirectPageReloadForExistingTabs = ({ liveClientCount = 0, matchingRuntimeClientCount = 0, } = {}) => liveClientCount > 0 && matchingRuntimeClientCount === 0;
export const runExistingTabsControllerReloadCommand = async ({ useDirectPageReload = false, controllerClientId, reason, enqueueCommand, }) => {
    ...
    return enqueueCommand(controllerClientId, 'reload-gemini-tabs', {
...
export const refreshExistingTabsExtensionRuntimeBeforeReload = async (args = {}, deps) => {
...
export const recoverExistingTabsContentScriptsAfterNativeReload = async (nativeReload, args = {}, deps) => {
    ...
    if (decision.state !== 'self_heal_content_scripts')
        return recovery;
    const selfHealArgs = {
        ...args,
        reason: decision.reason,
    ...
    const waitMs = deps.normalizeReloadWaitMs(args.reloadWaitMs ?? args.waitMs, deps.defaultWaitMs);
    const waitStartedAt = Date.now();
    const selfHeal = await deps.tryNativeBrowserBrokerTabsAction('selfHealContentScripts', selfHealArgs);
    const liveClients = decision.waitForClients
        ? await deps.waitForLiveClients(waitMs, deps.pollIntervalMs, deps.clientReadyAfterReload)
        : readyClients();
    return {
...
export const runExistingTabsNativeReloadRecovery = async (args = {}, deps) => {
    ...
    const nativeReload = managedReload?.ok === true
        ? managedReload
        : await deps.tryNativeBrowserBrokerTabsAction('reload', args);
    if (nativeReload?.reloadMode !== 'managed-tabs') {
        await deps.attachContentScriptSelfHealToNativeReload(nativeReload, args, deps.tryNativeBrowserBrokerTabsAction);
    }
...
export const reloadExtensionForExistingTabs = async (args = {}, ensureBrowserExtensionReady, summarizeClient, reloadTimeoutMs, normalizeReloadWaitMs, cleanupStaleClients) => {
    if (args.allowReload !== true)
        return null;
    try {
...
export const buildActivityClaimAffinity = (baseSessionId, activityClientId, existingGeminiSessionClaim = null, requestedVisualGroupTabId = null) => {
    const joinsExistingGeminiClaim = !!existingGeminiSessionClaim?.clientId &&
        existingGeminiSessionClaim.clientId !== activityClientId;
    return {
        sessionId: existingGeminiSessionClaim?.claimId && joinsExistingGeminiClaim
            ? `${baseSessionId}:activity:${existingGeminiSessionClaim.claimId}`
            : baseSessionId,
        visualGroupTabId: existingGeminiSessionClaim?.tabId && joinsExistingGeminiClaim
            ? existingGeminiSessionClaim.tabId
            : requestedVisualGroupTabId,
        joinsExistingGeminiClaim,
    };
};
export const normalizePositiveIntegerOrNull = (value, max = Number.MAX_SAFE_INTEGER) => {
    const parsed = Number(value);
...
export const previousWebConversationCountForResumeReport = (report, max) => normalizePositiveIntegerOrNull(report?.resume?.previousCounters?.webConversationCount, max) ??
    normalizePositiveIntegerOrNull(report?.webConversationCount, max);
export const recentChatsResumeCounters = (report, max) => ({
...
export const recentExportResumeScope = (args = {}, resume, exportMissingOnly, syncMode, maxChatsLoadTarget) => {
    const hasExplicitMaxChats = args.maxChats !== undefined || args.limit !== undefined;
    const resumeMaxChats = !exportMissingOnly && !syncMode
        ? normalizePositiveIntegerOrNull(resume?.previousCounters?.webConversationCount, maxChatsLoadTarget)
        : null;
    return {
        hasExplicitMaxChats,
        resumeMaxChats,
