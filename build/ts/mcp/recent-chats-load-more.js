...
export const recentChatsLoadMoreRuntimeConfig = (env = process.env) => ({
    loadMoreBudgetMs: finiteNumberOr(env.GEMINI_MCP_RECENT_CHATS_LOAD_MORE_BUDGET_MS, DEFAULT_RECENT_CHATS_LOAD_MORE_BUDGET_MS),
    loadMoreBrowserTimeoutMs: finiteNumberOr(env.GEMINI_MCP_RECENT_CHATS_LOAD_MORE_BROWSER_TIMEOUT_MS, DEFAULT_RECENT_CHATS_LOAD_MORE_BROWSER_TIMEOUT_MS),
});
export const normalizeRecentChatsLoadMorePlan = (currentCount, requestedLimit, args = {}, { defaultRounds = DEFAULT_RECENT_CHATS_LOAD_MORE_ROUNDS, defaultAttemptsPerRound = DEFAULT_RECENT_CHATS_LOAD_ATTEMPTS_PER_ROUND, maxRounds = 30, maxAttemptsPerRound = 5, maxTargetCount = MAX_RECENT_CHATS_LOAD_TARGET, } = {}) => {
    ...
    const attemptsPerRound = Math.max(1, Math.min(maxAttemptsPerRound, Number.isFinite(requestedAttempts) ? requestedAttempts : defaultAttemptsPerRound));
    return {
        loadedCount,
        targetCount,
        reachedEnd,
