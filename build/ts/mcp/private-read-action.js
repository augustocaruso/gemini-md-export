...
import { browserBackgroundChatReadCapability, buildChatReadAdapterPlan, domChatReadCapability, initialChatReadAdapterFallbackState, privateApiGeminiWebapiChatReadCapability, takeoutChatReadCapability, transitionChatReadAdapterFallback, } from '../core/chat-read-adapter.js';
...
const clientIdOf = (client) => {
    const value = client && typeof client === 'object' && 'clientId' in client ? client.clientId : null;
...
const usesGeminiWebapiPython = (args) => args.privateApiTransport === 'gemini-webapi-python';
const allowsPythonFallback = (args) => usesGeminiWebapiPython(args) || args.allowPythonFallback !== false;
const stringOrNull = (value) => {
    const text = String(value ?? '').trim();
    return text || null;
};
const timeoutMsOf = (value) => Math.max(5000, Math.min(120000, Number(value || 45000)));
...
const preferredAdapterForArgs = (args) => {
    if (args.privateApiTransport === 'browser-background')
        return 'browserBackground';
    if (usesGeminiWebapiPython(args))
        return 'privateApiGeminiWebapi';
    return 'browserBackground';
};
const strictBrowserBackgroundOnly = (args) => args.privateApiTransport === 'browser-background' &&
    args.allowPythonFallback === false &&
    args.allowDomFallback !== true;
const buildPrivateReadAdapterPlan = (args) => buildChatReadAdapterPlan({
    allowExperimental: true,
    preferredAdapter: preferredAdapterForArgs(args),
    capabilities: [
        privateApiGeminiWebapiChatReadCapability({
            available: allowsPythonFallback(args) && !!(parseChatId(args.chatId) || parseChatId(args.url)),
            canReadAssets: true,
...
const failureCodeOf = (adapter, result) => {
    const record = objectOrNull(result);
    return (stringOrNull(record?.code) ||
        stringOrNull(record?.reason) ||
        stringOrNull(record?.errorCode) ||
        `${adapter}_failed`);
};
const failureMessageOf = (adapter, result) => {
    if (result instanceof Error)
        return result.message;
    const record = objectOrNull(result);
    return (stringOrNull(record?.message) ||
        stringOrNull(record?.error) ||
        stringOrNull(record?.reason) ||
        `O adapter ${adapter} nao conseguiu ler a conversa.`);
};
const isOkResult = (result) => objectOrNull(result)?.ok === true;
const payloadOf = (result) => objectOrNull(result)?.payload;
const mediaFilesOfResult = (result) => {
    const record = objectOrNull(result);
    return Array.isArray(record?.mediaFiles) ? record.mediaFiles : [];
};
const expectedChatIdOf = (args) => parseChatId(args.chatId) || parseChatId(args.url);
const snapshotOf = (result) => {
    const snapshot = objectOrNull(objectOrNull(result)?.snapshot);
    if (!snapshot)
        return null;
    if (!parseChatId(snapshot.chatId) || !Array.isArray(snapshot.turns))
        return null;
    return snapshot;
};
const enrichSnapshotResult = (result) => {
...
export const createMcpPrivateReadAction = (deps) => async (args = {}) => {
    ...
    const getManagedClient = (purpose) => {
        if (!managedClient)
            managedClient = deps.requireManagedChatClient(args, purpose);
        return managedClient;
    };
    const readAdapter = async (adapter) => {
        ...
        if (adapter === 'browserBackground') {
            if (deps.nativeBrowserBroker?.privateApiReadChat) {
                ...
                const nativeResult = objectOrNull(nativeResponse)?.ok === true
                ...
                if (isOkResult(nativeResult)) {
                    ...
                    const assetRequestCount = Array.isArray(assetPlan?.requests)
                        ...
                        : 0;
                    if (args.downloadAssets === true &&
                        assetRequestCount > 0 &&
                        mediaFilesOfResult(enriched).length === 0) {
                        return {
                            ok: false,
                            code: 'browser_background_assets_unavailable',
                            message: 'A leitura pelo navegador encontrou assets, mas ainda nao consegue baixa-los; tentando sidecar Python.',
                            chatId: expectedChatIdOf(args),
                        };
                    }
                    return enriched;
                }
                if (strictBrowserBackgroundOnly(args))
                    return nativeResult;
            }
            const client = getManagedClient('private-api-read-chat');
            const command = buildPrivateApiReadChatCommand(args, client);
            const result = await deps.enqueueCommand(clientIdOf(client), 'private-api-read-chat', command.args, { timeoutMs: command.timeoutMs });
            const merged = {
        ...
        if (adapter === 'dom') {
            ...
            const result = await deps.enqueueCommand(clientIdOf(client), commandType, {
    ...
    return {
        ...
        message: warnings.at(-1)?.message || 'Nenhum adapter disponivel conseguiu ler a conversa do Gemini.',
        adapterPlan,
        fallbackWarnings: warnings,
        adapterAttempts: state.attempts,
    };
};
export const createMcpPrivateReadRuntimes = (deps) => {
