import { runGeminiWebapiPythonListChats, runGeminiWebapiPythonSessionStatus, } from './gemini-webapi-python-adapter.js';
export const privateInventoryLimit = (args = {}, fallback = 200) => Math.max(1, Math.min(2000, Number(args.limit || args.maxChats || fallback)));
const allowsDomFallback = (args = {}) => args.allowDomFallback === true;
const conversationFromChat = (chat = {}, source = 'private-api', deps) => {
    const chatId = deps.normalizeConversationChatId(chat) ||
...
const applyPrivateInventoryToClient = (client, result, args) => {
    if (!client)
        return;
...
const listViaBrowserBackground = async (client, args, deps) => {
    const timeoutMs = Math.max(5000, Number(args.privateInventoryWaitMs || args.waitMs || 30_000));
    const result = await deps.enqueueCommand(client.clientId, 'private-api-list-chats', {
...
const listViaNativeBrowserBroker = async (args, deps) => {
    if (!deps.nativeBrowserBroker?.privateApiListChats)
        return null;
    const timeoutMs = Math.max(5000, Number(args.privateInventoryWaitMs || args.waitMs || 30_000));
    ...
    if (!result?.ok)
        return result || { ok: false, code: 'native_private_inventory_failed' };
    const conversations = (Array.isArray(result.chats) ? result.chats : [])
        .map((chat) => conversationFromChat(chat, 'private-api-browser', deps))
        .filter(Boolean);
    return {
...
const listViaPythonSidecar = async (args, deps) => {
...
export const listPrivateChatsForClient = async (client, args = {}, deps) => {
    if (args.privateInventory === false)
        return null;
    const attempts = [];
    try {
...
export const privateRecentChatsResponse = (client, inventory, args, deps) => {
    ...
    return {
        ...
        nextAction: reachedEnd
            ...
            : {
                ...
                message: 'A API privada retornou um lote parcial. Use um limite maior ou espere a implementacao de paginacao privada antes de afirmar total absoluto.',
                command: null,
            },
    };
};
export const listPrivateRecentChatsForClient = async (client, args, context, deps) => {
    const targetCount = Math.max(1, Number(context.targetCount || args.limit || 10));
    const inventory = await listPrivateChatsForClient(client, {
        ...args,
        limit: Math.max(targetCount, privateInventoryLimit(args, targetCount)),
    }, deps);
    if (!inventory?.ok)
        return null;
    return privateRecentChatsResponse(client, inventory, {
        ...args,
        ...context,
        targetCount,
    }, deps);
};
export const loadPrivateInventoryForRecentExportJob = async (job, client, args, deps) => {
    ...
    const privateInventory = await listPrivateChatsForClient(client, {
        ...args,
        limit: privateLimit,
    }, deps.inventoryDeps);
    if (!privateInventory?.ok)
        return null;
...
export const privateSupportLegacyToolName = (action) => supportLegacyToolNames[String(action || 'diagnose')] || supportLegacyToolNames.diagnose;
const commandReadyGeminiClients = (deps) => typeof deps.getCommandReadyGeminiClients === 'function'
    ? deps
        .getCommandReadyGeminiClients()
        .filter((client) => deps.commandChannelReadyForClient(client))
    : [];
const clientMatchesSessionSelector = (client, selector) => {
    if (selector.clientId)
        return client?.clientId === selector.clientId;
    if (selector.tabId !== null && selector.tabId !== undefined) {
        return String(client?.tabId ?? '') === String(selector.tabId);
    }
    return false;
};
const browserClientForSessionStatus = (args, deps) => {
    ...
    if (selector.clientId ||
        ...
        try {
            ...
            const readyMatch = readyClients.find((client) => clientMatchesSessionSelector(client, selector));
            if (readyMatch)
                return readyMatch;
            throw err;
        }
    }
    return (deps.getSelectableGeminiClients().find((client) => deps.commandChannelReadyForClient(client)) ||
        readyClients[0] ||
        null);
};
...
const browserOnlySessionNextAction = (attempts, browserClient) => {
    const nativeRuntimeDisconnected = !browserClient &&
        attempts.some((attempt) => attempt.adapter === 'browserBackground' &&
    ...
    if (nativeRuntimeDisconnected) {
        return {
            ...
            message: 'Recarregue a extensao e a aba do Gemini no navegador logado; depois rode auth status novamente.',
        };
    }
    return {
...
export const checkPrivateSessionStatus = async (args = {}, deps) => {
    const attempts = [];
    if (deps.nativeBrowserBroker?.privateApiSessionStatus) {
        try {
            const timeoutMs = Math.max(5000, Number(args.waitMs || 20_000));
            const nativeResponse = await deps.nativeBrowserBroker.privateApiSessionStatus({ timeoutMs }, { allowFallback: true });
    ...
    if (browserClient?.clientId && deps.commandChannelReadyForClient(browserClient)) {
        try {
            const timeoutMs = Math.max(5000, Number(args.waitMs || 20_000));
            const browserResult = await deps.enqueueCommand(browserClient.clientId, 'private-api-session-status', { timeoutMs }, { timeoutMs });
            attempts.push({
                adapter: 'browserBackground',
    ...
    const pythonRecord = pythonResult;
    attempts.push({
        adapter: 'privateApiGeminiWebapi',
    ...
    return {
        ...
        nextAction: {
            ...
            message: 'Abra o Gemini no navegador logado ou atualize os cookies usados pelo sidecar Python.',
        },
    };
};
