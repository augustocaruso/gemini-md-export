...
export const summarizeMetadataBackfillItems = ({ totalChats, filesRewritten, items, }) => {
...
export const buildMetadataBackfillContract = ({ summary, items, activityError = null, }) => {
    if (summary.complete) {
        return {
            ok: true,
            status: 'complete',
    ...
    const hasUnresolved = pendingItems.some((item) => item.status === 'unresolved' || item.status === 'partial');
...
export const metadataBackfillStatusForDates = (dates) => {
    ...
    if (hasCreated && hasLastMessage)
        return 'matched';
    return hasCreated || hasLastMessage ? 'partial' : 'unresolved';
};
export const metadataCandidateHasCompleteDates = (candidate, match) => metadataBackfillStatusForDates({
    dateCreated: match?.dateCreated || candidate.dateCreated,
    dateLastMessage: match?.dateLastMessage || candidate.dateLastMessage,
}) === 'matched';
export const filterCandidatesMissingMetadataDates = (candidates, groupedMatches) => candidates.filter((candidate) => !metadataCandidateHasCompleteDates(candidate, groupedMatches.get(candidate.chatId)));
export const metadataBackfillHumanSummary = (summary, contract) => {
    ...
    const pendingLabel = contract.code === 'takeout_source_mismatch'
        ? 'fonte incompatível'
...
export const metadataBackfillStepStatus = ({ exitCode, contract = null, }) => {
    if (contract?.status === 'blocked')
        return 'blocked';
    return exitCode === 0 ? 'completed' : 'failed';
};
export const metadataBackfillWarning = (contract) => {
    if (contract?.status !== 'blocked')
        return null;
    return {
        code: contract.code,
        ...
        nextAction: contract.code === 'raw_export_suspected'
...
export const buildFixVaultMetadataStatus = ({ exitCode, report = null, }) => {
    ...
    if (activityError) {
        warnings.push({
            ...
            nextAction: 'Abra https://myactivity.google.com/product/gemini, confirme que a extensao foi recarregada e rode fix-vault de novo para datas remanescentes.',
        });
    }
