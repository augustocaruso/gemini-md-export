...
export const buildProgressViewModel = (input) => {
...
export const buildExportJobProgressViewModel = (job) => {
...
export const buildGuiExportProgressViewModel = (progress) => buildProgressViewModel({
...
export const buildActivityProgressViewModel = (progress) => {
...
export const buildFixVaultProgressViewModel = ({ current, total, message, }) => buildProgressViewModel({
...
export const mergeProgressViewModel = (previous, next) => {
    if (!previous)
        return next;
    if (next.terminal)
        return next;
...
export const progressCreepCeiling = (view) => {
...
export const normalizeProgressDisplayPercent = (previous, next, previousDisplayPercent = previous?.displayPercent ?? 0) => {
