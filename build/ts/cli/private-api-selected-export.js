...
import { ensureGeminiWebapiPythonBootstrap, runGeminiWebapiPythonListChats, runGeminiWebapiPythonReadChat, runGeminiWebapiPythonSessionStatus, } from '../mcp/gemini-webapi-python-adapter.js';
...
const defaultDeps = {
    ...
    ensureBrowserKeepAlive: async (input) => (await createNativeBrowserBrokerClient().keepAlive(input, {
        allowFallback: true,
    })),
    recoverBrowserBackgroundSession: async ({ args, timeoutMs }) => {
        const waitMs = Math.max(5000, Math.min(timeoutMs, 30_000));
        await bridgeJsonFetch(args.bridgeUrl, appendParams('/agent/ready', {
            detail: 'compact',
    ...
    runBrowserBackgroundReadChat: async ({ args, item, timeoutMs }) => {
        ...
        try {
            ...
            const nativeResult = nativeResponse?.ok === true
                ? nativeResponse.result
                : nativeResponse;
            if (nativeResult?.ok === true)
                return nativeResult;
            throw Object.assign(new Error(readableText(nativeResult?.message) ||
                readableText(nativeResult?.error) ||
                'A leitura privada pelo native broker falhou.'), { code: stringOrNull(nativeResult?.code) || 'native_broker_private_read_failed' });
        }
        catch {
...
const shouldAttemptBridgePrivateExport = (args, items) => {
    if (args.preferBridge === false)
        return false;
    if (stringOrNull(args.python) || stringOrNull(args.cookiesJson))
        return false;
    if (stringOrNull(process.env.GME_GEMINI_WEBAPI_RUNNER))
        return false;
    if (args.recent === true)
        return false;
    if (items.length === 0)
...
const readableText = (value) => {
    ...
    if (value instanceof Error)
        return stringOrNull(value.message);
...
const resolveOutputDir = (outputDir) => {
    const raw = stringOrNull(outputDir) || process.env.GEMINI_MCP_EXPORT_DIR || resolve(homedir(), 'Downloads');
...
const bridgeJsonFetch = async (bridgeUrl, path, { method = 'GET', body, timeoutMs = 15_000, } = {}) => {
    ...
    try {
        ...
        if (!response.ok) {
            throw Object.assign(new Error(stringOrNull(json.message) ||
        ...
        throw Object.assign(new Error(err instanceof Error
            ...
            : 'Bridge local indisponivel para leitura privada.'), { code: 'bridge_private_read_unavailable', cause: err });
    }
    finally {
...
const bridgePrivateReadClientId = async (args, timeoutMs) => {
    const explicit = stringOrNull(args.clientId);
    if (explicit)
        return explicit;
    const ready = await bridgeJsonFetch(args.bridgeUrl, appendParams('/agent/ready', {
        wakeBrowser: false,
    ...
    return stringOrNull(clients.find((client) => stringOrNull(client?.clientId))?.clientId);
};
const toolTextPayload = (toolCallResponse) => {
    const result = toolCallResponse.result && typeof toolCallResponse.result === 'object'
    ...
    const text = stringOrNull(content.find((item) => item?.type === 'text')?.text);
    if (text) {
        try {
...
const readChatViaBridgePrivateRead = async ({ args, item, timeoutMs, }) => {
    ...
    if (payload.ok !== true) {
        throw Object.assign(new Error(readableText(payload.message) ||
            readableText(payload.error) ||
            'A leitura privada pela bridge falhou.'), { code: stringOrNull(payload.code) || 'bridge_private_read_failed', data: payload });
    }
    return payload;
};
...
const bufferFromPayload = (payload, emptyMessage, emptyCode) => {
    const content = typeof payload.contentBase64 === 'string'
...
const writePayloadFile = (payload, outputDir, fallbackFilename, emptyMessage, emptyCode) => {
...
const writeMarkdownPayload = (payload, outputDir) => writePayloadFile(payload, outputDir, `${payload.chatId || 'gemini-chat'}.md`, 'O adapter privado nao retornou Markdown para salvar.', 'private_api_empty_markdown');
const mediaFilesOf = (payload) => Array.isArray(payload.mediaFiles)
    ? payload.mediaFiles.filter((item) => item !== null && typeof item === 'object')
    : [];
const mediaFailuresOf = (payload) => Array.isArray(payload.mediaFailures) ? payload.mediaFailures : [];
const payloadWithLocalMediaLinks = (payload, mediaFiles) => {
...
const writePayloadBundle = (payload, outputDir) => {
    const mediaPayloads = mediaFilesOf(payload);
    const markdown = writeMarkdownPayload(payloadWithLocalMediaLinks(payload, mediaPayloads), outputDir);
    const mediaFiles = mediaPayloads.map((file) => writePayloadFile(file, outputDir, String(file.filename || 'asset.bin'), 'O adapter privado retornou um asset vazio.', 'private_api_empty_asset'));
    return {
        ...markdown,
        mediaFiles,
...
const payloadWithFilename = (payload, filename) => filename ? { ...payload, filename } : payload;
const makeJob = ({ jobId, status, outputDir, requested, completed, current, savedFiles, failures, progressMessage, startedAt, updatedAt, finishedAt, }) => ({
    jobId,
...
const failureFromError = (item, index, err) => {
    const record = err && typeof err === 'object' ? err : {};
    return {
        index,
        chatId: item.chatId || null,
        title: item.title || null,
        error: readableText(err instanceof Error ? err.message : record.message || err) ||
            'erro sem detalhe',
        code: stringOrNull(record.code) || null,
    };
};
const isTransientPrivateReadError = (err) => {
    const record = err && typeof err === 'object' ? err : {};
    const code = stringOrNull(record.code) || '';
...
const browserKeepAliveMsForBatch = ({ args, requested, readTimeoutMs, }) => {
...
const normalizeItems = (args) => {
    const maxItems = numberInRange(args.maxItems, 500, 1, 1000);
    const selection = normalizeDirectReexportSelection(args, { maxItems });
...
const normalizePrivateRecentChatItem = (chat) => {
    const chatId = stringOrNull(chat.chatId || chat.chat_id || chat.privateChatId || chat.private_chat_id || chat.id)?.replace(/^c_/, '');
    if (!chatId)
        return null;
    return {
        chatId,
...
const listRecentItems = async (args, deps) => {
    ...
    if (result?.ok !== true) {
        throw Object.assign(new Error(stringOrNull(result?.message) ||
            stringOrNull(result?.error) ||
            'Nao consegui listar conversas pela API privada.'), { code: stringOrNull(result?.code) || 'private_inventory_unavailable' });
    }
...
export const runPrivateApiSelectedExport = async (args, deps = {}) => {
    ...
    let requested = Math.max(items.length, numberInRange(args.expectedCount ?? (args.recent === true ? args.limit : 0), args.recent === true ? 10 : 0, 0, 2000));
    const savedFiles = [];
    const failures = [];
    const startedAt = runtimeDeps.now().toISOString();
    const jobId = `private-api-${Date.now().toString(36)}`;
    const emit = (status, current, progressMessage, finishedAt) => {
        const job = makeJob({
            jobId,
            status,
            outputDir,
    ...
    if (!useBridgePrivateRead) {
        const bootstrapTimeoutMs = numberInRange(args.bootstrapTimeoutMs ?? process.env.GME_GEMINI_WEBAPI_BOOTSTRAP_TIMEOUT_MS, 180_000, 30_000, 300_000);
        ...
        if (!bootstrapResult.ok) {
            ...
            return emit('failed', null, 'Preparacao da API privada falhou', runtimeDeps.now().toISOString());
        }
        ...
        if (sessionResult?.ok !== true || sessionResult.authenticated !== true) {
            failures.push({
                ...
                chatId: null,
                title: null,
                error: stringOrNull(sessionResult?.message) ||
            ...
            return emit('failed', null, 'Sessão da API privada precisa de login', runtimeDeps.now().toISOString());
        }
    }
    if (items.length === 0 && args.recent === true) {
        ...
        try {
            ...
            return emit('failed', null, 'Listagem pela API privada falhou', runtimeDeps.now().toISOString());
        }
    }
    if (items.length === 0) {
        throw Object.assign(new Error('Nenhuma conversa selecionada para exportar.'), {
    ...
    const readTimeoutMs = numberInRange(args.privateReadWaitMs ?? args.waitMs ?? args.timeoutMs, 45_000, 5_000, 120_000);
    const maxReadAttempts = numberInRange(args.maxReadAttempts ?? (useBridgePrivateRead ? process.env.GME_PRIVATE_READ_MAX_ATTEMPTS : 1), useBridgePrivateRead ? 2 : 1, 1, 5);
    ...
    for (const [index, item] of items.entries()) {
        ...
        try {
            ...
            for (let attempt = 1; attempt <= maxReadAttempts; attempt += 1) {
                try {
                    ...
                    if (readResult?.ok === false) {
                        ...
                        throw Object.assign(new Error(readableText(failure.message) ||
            ...
            const collected = privateReadExportResultToCollectedPayload({
                ...
                result: {
                    ...readResult,
                    adapter: stringOrNull(readResult.adapter) ||
            ...
            const saved = writePayloadBundle(payloadWithFilename(collected.result.payload, item.filename || null), itemOutputDir);
            savedFiles.push({
    ...
    const status = failures.length === 0
        ...
        : savedFiles.length > 0
            ...
            : 'failed';
    latestJob = emit(status, null, status === 'completed'
        ? 'Export privado concluido'
        : status === 'completed_with_errors'
            ? 'Export privado concluido com falhas'
            : 'Export privado falhou', finishedAt);
    return latestJob;
};
export const summarizePrivateApiSelectedExportJob = (job) => ({
