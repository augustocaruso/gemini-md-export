...
const nativeBrokerDebuggableTabs = (ready) => {
    const tabs = ready.nativeBroker?.response?.result?.tabs;
    if (!Array.isArray(tabs))
        return null;
    return tabs.filter((item) => item?.state === 'debuggable');
};
const nativeBrokerCanReloadExistingTab = (ready) => {
    if (ready.nativeBroker?.available !== true)
        return false;
...
const clientLooksLikeGeminiApp = (client) => {
    const kind = pageKind(client);
    if (kind === 'chat' || kind === 'gemini' || kind === 'notebook')
        return true;
    if (kind === 'activity')
        return false;
...
const knownClientsFromReady = (ready) => {
    ...
    if (Array.isArray(ready.connectedClients))
        return ready.connectedClients;
    if (Array.isArray(ready.clients))
        return ready.clients;
    return [];
};
const readyHasKnownOnlyNonGeminiClients = (ready) => {
    const clients = knownClientsFromReady(ready);
    return clients.length > 0 && !clients.some(clientLooksLikeGeminiApp);
};
export const shouldWakeBrowserForReady = (ready = {}) => {
    if (ready.ready === true)
        return false;
    ...
    if (issue === 'no_connected_clients')
        return true;
    if (issue === 'no_selectable_gemini_tab') {
        if (readyHasKnownOnlyNonGeminiClients(ready))
            return true;
        return connected <= 0 && selectable <= 0;
    }
    return connected <= 0 && selectable <= 0;
};
export const shouldWaitForExistingTabsForReady = (ready = {}) => {
    if (ready.ready === true)
        return false;
    if (readyHasKnownOnlyNonGeminiClients(ready))
        return false;
...
export const shouldReloadAllExistingTabsForReady = (ready = {}, flags = {}) => {
    if (flags.allowReload !== true || ready.ready === true)
        return false;
    if (blockingIssueCode(ready.blockingIssue) !== 'no_connected_clients')
        return false;
    if (ready.nativeBroker?.available !== true)
        return false;
    const tabs = nativeBrokerDebuggableTabs(ready);
...
export const shouldReloadExistingTabsForReady = (ready = {}, flags = {}) => {
    if (flags.allowReload !== true || ready.ready === true)
        return false;
    const issue = blockingIssueCode(ready.blockingIssue);
    ...
    if (connectedClientCountFromReady(ready) > 0 &&
