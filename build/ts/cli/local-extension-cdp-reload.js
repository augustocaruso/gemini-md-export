...
import { devToolsActivePortPathForUserDataDir, reloadExtensionFromDevToolsActivePort, } from '../cdp/browser-websocket.js';
const normalizeBrowser = (value) => {
    ...
    if (!text)
        return 'chrome';
    if (/^(google[-_\s]*)?chrome$|chrome\.exe$/.test(text))
        return 'chrome';
    if (/^edge$|microsoft[-_\s]*edge|msedge(\.exe)?$/.test(text))
        return 'edge';
    if (/^brave$|brave[-_\s]*browser|brave(\.exe)?$/.test(text))
        return 'brave';
    if (/^dia$|dia(\.exe)?$/.test(text))
        return 'dia';
    return text;
};
export const browserUserDataDirForLocalCdp = ({ browser = 'chrome', platform = nodePlatform(), homeDir = homedir(), } = {}) => {
    const key = normalizeBrowser(browser);
    if (platform === 'darwin') {
        const base = resolve(homeDir, 'Library', 'Application Support');
        if (key === 'dia')
            return resolve(base, 'Dia', 'User Data');
        if (key === 'edge')
            return resolve(base, 'Microsoft Edge');
        if (key === 'brave')
            return resolve(base, 'BraveSoftware', 'Brave-Browser');
        if (key === 'chrome')
    ...
    if (platform === 'win32') {
        ...
        if (key === 'dia')
            return resolve(localAppData, 'Dia', 'User Data');
        if (key === 'edge')
            return resolve(localAppData, 'Microsoft', 'Edge', 'User Data');
        if (key === 'brave')
    ...
    if (key === 'dia')
        return resolve(configHome, 'Dia');
    if (key === 'edge')
        return resolve(configHome, 'microsoft-edge');
    if (key === 'brave')
        return resolve(configHome, 'BraveSoftware', 'Brave-Browser');
    if (key === 'chrome')
        return resolve(configHome, 'google-chrome');
    return null;
};
export const runLocalExtensionCdpReload = async (input, deps = {}) => {
    const mode = 'cdp-browser-websocket';
    ...
    const reload = deps.reloadExtensionFromDevToolsActivePort || reloadExtensionFromDevToolsActivePort;
