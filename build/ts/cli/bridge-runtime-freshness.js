...
const runtimeFreshnessFilesForRoot = (root) => [
...
const newestRuntimeFileForRoot = (root) => {
    if (!root)
        return null;
    let newest = null;
...
export const staleBridgeMessage = (mismatch, { safe = false, version }) => {
    ...
    if (mismatch?.kind === 'protocol') {
        return (`Bridge local desatualizada${pid}: protocolo ${mismatch.actualProtocolVersion ?? '?'}; ` +
            `esta CLI espera protocolo ${mismatch.expectedProtocolVersion ?? '?'}.${location}${restartSuffix}`);
    }
    if (mismatch?.kind === 'browser-build') {
        return (`Bridge local desatualizada${pid}: build da extensão ${mismatch.actualBuildStamp ?? '?'}; ` +
            `esta CLI espera ${mismatch.expectedBuildStamp ?? '?'}.${location}${restartSuffix}`);
    }
    if (mismatch?.kind === 'runtime-files') {
        return (`Bridge local desatualizada${pid}: arquivos instalados mudaram depois que o processo iniciou.` +
    ...
    return (`Bridge local desatualizada${pid}: ${mismatch?.actualVersion || '?'} -> ${mismatch?.expectedVersion || version}.` +
        `${location}${restartSuffix}`);
};
export const bridgeMismatchStatusDetail = (mismatch, version) => {
    if (mismatch?.kind === 'browser-build') {
...
export const bridgeMismatchIntervalMessage = (mismatch, elapsedText, version) => `${bridgeMismatchStatusDetail(mismatch, version).replace(/\.$/, '')}; ${elapsedText} decorridos.`;
