...
import { assistantFragments, candidateNeedles, groupMetadataEvidence, normalizeComparableText, sampleText, } from '../core/metadata-evidence.js';
...
const TAKEOUT_MONTHS_EN = new Map([
    ['jan', 1],
    ['january', 1],
    ['feb', 2],
    ['february', 2],
...
const parseTakeoutZoneOffsetMinutes = (zoneText) => {
...
const portableIsoFromTakeoutParts = ({ year, month, day, hour, minute, second, offsetMinutes, }) => portableIsoSeconds(new Date(Date.UTC(year, month - 1, day, hour, minute, second) - offsetMinutes * 60_000));
export const parseTakeoutDate = (text) => {
    const normalized = String(text || '').replace(/[\u00a0\u202f]/g, ' ');
    const ptMatch = normalized.match(/(\d{1,2})\s+de\s+([A-Za-zÀ-ÿ.]+)\s+de\s+(\d{4}),\s+(\d{1,2}):(\d{2}):(\d{2})\s+([A-Z]{2,5})/i);
    if (ptMatch) {
    ...
    const enMatch = normalized.match(/([A-Za-z]{3,9})\s+(\d{1,2}),\s+(\d{4}),\s+(\d{1,2}):(\d{2}):(\d{2})\s+(AM|PM)?\s*(GMT|UTC)?([+-]\d{1,2}:?\d{2})?/i);
    if (!enMatch)
        return null;
    const [, monthText, dayText, yearText, hourText, minuteText, secondText, meridiemText, zonePrefix, offsetText,] = enMatch;
...
export const parseTakeoutHtmlItems = (html) => {
    ...
    return cards
        ...
        if (!date || !/Gemini Apps/i.test(text))
            return null;
        return {
            date,
            text,
...
const collectTakeoutObjects = (value, out = []) => {
    if (Array.isArray(value)) {
        for (const item of value)
            collectTakeoutObjects(item, out);
        return out;
    }
...
const matchTakeoutHtmlItems = (items, candidates) => {
    ...
    return preferExactPromptTakeoutMatches(dedupeTakeoutEvidence([...matches, ...matchTakeoutCandidateEdges(items, candidates)]));
};
const preferExactPromptTakeoutMatches = (matches) => {
    const exactPromptKindsByChatId = new Map();
    const assistantSupportedExactDateByChatKind = new Map();
    const exactMatchesByChatKind = new Map();
    for (const match of matches) {
        if (!match.warnings?.includes('takeout_prompt_exact'))
            continue;
        const chatId = String(match.chatId || '').toLowerCase();
        const kinds = exactPromptKindsByChatId.get(chatId) || new Set();
    ...
    for (const [key, exactMatches] of exactMatchesByChatKind.entries()) {
        const supportedDates = Array.from(new Set(exactMatches
            .filter((match) => match.warnings?.includes('takeout_assistant_hit'))
            .map((match) => match.date)
            .filter(Boolean)));
        if (supportedDates.length === 1)
            assistantSupportedExactDateByChatKind.set(key, supportedDates[0]);
    }
    if (!exactPromptKindsByChatId.size)
        return matches;
    return matches.filter((match) => {
        const chatId = String(match.chatId || '').toLowerCase();
        const assistantSupportedDate = assistantSupportedExactDateByChatKind.get(`${chatId}:${match.dateKind}`);
        if (assistantSupportedDate &&
        ...
        if (match.warnings?.includes('takeout_prompt_exact'))
            return true;
        if (!match.warnings?.includes('takeout_prompt_contains'))
            return true;
        if (match.dateKind !== 'unknown')
            return true;
        return !exactKinds.has('unknown');
    });
};
const dedupeTakeoutEvidence = (matches) => {
    const byKey = new Map();
    const rank = (match) => Number(match.score || 0) +
    ...
    for (const match of matches) {
        ...
        if (!current || rank(match) > rank(current))
            byKey.set(key, match);
    }
    return Array.from(byKey.values());
};
const edgeRefsForCandidate = (candidate) => {
    if (!Number.isFinite(candidate.turnCount) || Number(candidate.turnCount) <= 1)
        return [];
    const refs = [];
    const add = (kind, prompt, assistantText) => {
...
const uniqueDates = (hits) => Array.from(new Set(hits.map((hit) => hit.item.date))).sort();
const pickEdgeHit = (hits) => {
    if (!hits.length)
        return null;
...
const matchTakeoutCandidateEdges = (items, candidates) => {
    const edgeRefsByPattern = new Map();
    ...
    if (!edgeRefsByPattern.size)
        return [];
    const automaton = new AhoCorasick(Array.from(edgeRefsByPattern.keys()).map((pattern, index) => ({
        id: String(index),
        pattern,
        value: pattern,
    })));
    const hitsByChatEdge = new Map();
    for (const item of items) {
        ...
        for (const pattern of patterns) {
            for (const edge of edgeRefsByPattern.get(pattern) || []) {
                ...
                if (promptMatch === 'contains' && edge.length < 24)
                    continue;
                const assistantSupported = edge.assistantComparableFragments.some((fragment) => item.comparableText.includes(fragment));
    ...
    for (const hits of hitsByChatEdge.values()) {
        ...
        evidence.push({
            chatId: picked.edge.chatId,
            source: 'takeout-html',
            kind: picked.edge.kind,
            dateKind: picked.edge.kind,
            confidence: 'strong',
            date: picked.item.date,
...
const buildTakeoutTextMatcher = (candidates) => {
    const promptHitsByPattern = new Map();
    const fullTextHitsByPattern = new Map();
    for (const candidate of candidates) {
    ...
    const distinctCandidateCountByPattern = new Map([...promptHitsByPattern.entries(), ...fullTextHitsByPattern.entries()].map(([pattern, hits]) => [pattern, new Set(hits.map((hit) => hit.chatId)).size]));
    return (item) => {
        const foundPromptPatterns = new Set(promptAutomaton.search(item.promptComparableText).map((match) => match.value));
        const foundFullTextPatterns = new Set(fullTextAutomaton.search(item.comparableText).map((match) => match.value));
        if (!foundPromptPatterns.size && !foundFullTextPatterns.size)
            return [];
        const byCandidate = new Map();
        const addHits = (patterns, hitsByPattern) => {
            for (const pattern of patterns) {
                for (const hit of hitsByPattern.get(pattern) || []) {
                    const promptMatch = hit.haystack === 'prompt'
                        ? item.promptComparableText === hit.comparable
                            ? 'exact'
                            : 'contains'
                        : undefined;
                    if (hit.requiresPromptComparable &&
                    ...
                    const effectiveWeight = promptMatch === 'exact' && (hit.kind === 'created' || hit.kind === 'last_message')
        ...
        const independentlyAnchored = scored.filter(({ score, rawScore }) => {
            if (rawScore < 0.62)
                return false;
            const hasUniqueEdgePrompt = score.hits.some((hit) => (hit.kind === 'created' || hit.kind === 'last_message') &&
                Boolean(hit.promptMatch) &&
                hit.length >= 24 &&
                distinctCandidateCountByPattern.get(hit.comparable) === 1);
            const hasEdgePrompt = score.hits.some((hit) => (hit.kind === 'created' || hit.kind === 'last_message') &&
                !hit.requiresPromptComparable &&
                hit.promptMatch === 'exact' &&
                hit.weight >= 0.62);
            const hasEdgeAssistant = score.hits.some((hit) => (hit.kind === 'created' || hit.kind === 'last_message') &&
                (hit.requiresPromptComparable || hit.weight < 0.62));
            const hasIndependentAssistant = score.hits.some((hit) => hit.kind === 'assistant' &&
                hit.length >= 80 &&
                distinctCandidateCountByPattern.get(hit.comparable) === 1);
            if (hasUniqueEdgePrompt)
                return true;
            if (rawScore < 1.04)
                return false;
        ...
        if (runnerUp && runnerUp.rawScore >= best.rawScore - 0.05)
            return [];
        return [best.evidence];
    };
};
const evidenceFromTakeoutScore = ({ item, score, candidateCount, distinctCandidateCountByPattern, }) => {
    const promptHits = score.hits.filter((hit) => hit.kind === 'created' || hit.kind === 'last_message');
    ...
    const hasUsableExactPrompt = promptHits.some((hit) => hit.promptMatch === 'exact' && (candidateCount === 1 || hit.length >= 24));
    const uniquePromptOnly = promptHits.some((hit) => Boolean(hit.promptMatch) &&
        hit.length >= 24 &&
        distinctCandidateCountByPattern.get(hit.comparable) === 1);
    const longUniqueAssistant = assistantHits.some((hit) => hit.length >= 80 && distinctCandidateCountByPattern.get(hit.comparable) === 1);
    ...
    const promptMatchWarnings = Array.from(new Set([
        ...promptHits.map((hit) => hit.promptMatch === 'exact'
...
const loadTakeoutJsonMatchesFromText = (text) => {
    const parsed = JSON.parse(text);
    const matches = [];
    for (const item of collectTakeoutObjects(parsed)) {
        const chatId = String(item.chatId || item.chat_id || '').match(CHAT_ID_RE)?.[0] ||
            String(item.url || item.link || item.titleUrl || '').match(URL_CHAT_ID_RE)?.[1] ||
            String(item.href || '').match(URL_CHAT_ID_RE)?.[1];
        const date = portableIsoSeconds(item.date || item.timestamp || item.time || item.time_usec || item.createdAt);
        if (!chatId || !date)
            continue;
        matches.push({
            ...
            dateKind: item.kind === 'created' || item.kind === 'last_message'
                ? item.kind
                : 'unknown',
            confidence: 'strong',
            source: 'takeout-json',
...
const loadTakeoutJsonMatches = (path) => loadTakeoutJsonMatchesFromText(readFileSync(path, 'utf-8'));
const isTakeoutHtmlEntry = (name) => /\.html?$/i.test(name);
const isTakeoutJsonEntry = (name) => /\.json$/i.test(name);
export const matchTakeoutItems = (items, candidates = []) => matchTakeoutHtmlItems(items, candidates);
export const matchTakeoutSource = (source, candidates = []) => {
    const candidateIds = new Set(candidates.map((candidate) => candidate.chatId.toLowerCase()));
    const jsonMatches = source.jsonMatches.filter((match) => candidateIds.size ? candidateIds.has(String(match.chatId || '').toLowerCase()) : true);
    return [...matchTakeoutItems(source.htmlItems, candidates), ...jsonMatches];
};
export const loadTakeoutSource = (path) => {
...
export const loadTakeoutMatches = (path, candidates = []) => {
    if (!path)
        return [];
...
export const loadTakeoutEvidence = ({ takeoutPath, candidates, }) => {
    if (!takeoutPath)
        return emptyTakeoutEvidence('');
    ...
    for (const candidate of candidates) {
        ...
        byChatId.set(candidate.chatId, match
            ? {
                ...match,
