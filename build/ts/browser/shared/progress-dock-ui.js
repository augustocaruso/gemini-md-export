...
export const ensureSharedProgressDock = ({ dockId = SHARED_PROGRESS_DOCK_ID, initialTitle = '', documentRef = document, } = {}) => {
...
export const applySharedProgressDockTheme = (dock, { dark = false, documentRef = document } = {}) => {
    if (!dock)
        return;
    applyCssVars(dock, buildDockHostPalette({ documentRef, isDark: dark }));
};
export const getSharedProgressDockElements = ({ dockId = SHARED_PROGRESS_DOCK_ID, documentRef = document, } = {}) => ({
