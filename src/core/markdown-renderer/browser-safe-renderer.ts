...
const decodeHtmlEntities = (value: string): string =>
  value.replace(/&(#x[0-9a-f]+|#\d+|[a-z][a-z0-9]+);/gi, (entity, rawName: string) => {
...
const hrefFromAttributes = (attributes: string): string | null => {
...
const renderInline = (value: string): string => {
  ...
  html = html.replace(
    /<(strong|b)\b[^>]*>([\s\S]*?)<\/\1>/gi,
    (_match, _tag, inner) => `**${renderInline(inner)}**`,
  );
  html = html.replace(
    /<(em|i)\b[^>]*>([\s\S]*?)<\/\1>/gi,
    (_match, _tag, inner) => `_${renderInline(inner)}_`,
  );
  html = html.replace(
    /<code\b[^>]*>([\s\S]*?)<\/code>/gi,
    (_match, inner) => `\`${decodeHtmlEntities(stripTags(String(inner || ''))).trim()}\``,
  );
...
const renderHtmlFragment = (value: string): string => {
  const placeholders: string[] = [];
  const stash = (markdown: string): string => {
...
export const createBrowserSafeMarkdownRenderer = (): MarkdownRenderer => ({
  render(input: MarkdownInput | unknown): string {
    const normalized = normalizeMarkdownInput(input);
    if (normalized.format === 'html') return renderHtmlFragment(normalized.value);
