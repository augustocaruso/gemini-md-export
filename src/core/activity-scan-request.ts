export type MetadataActivityScanCandidate = {
  chatId: string;
  scoring: {
    title?: string;
    firstPrompt?: string;
...
export type MetadataActivityScanRequest = {
  bridgeUrl: string;
  candidates: MetadataActivityScanCandidate[];
  resume?: unknown;
...
export const requestMetadataActivityScan = async ({
  bridgeUrl,
  candidates,
  resume = null,
  openIfMissing = true,
  timeoutMs = metadataActivityScanTimeoutMs(),
  fetchImpl = fetch,
}: MetadataActivityScanRequest) => {
  ...
  try {
    ...
    if ((err as { name?: string }).name === 'AbortError') {
      const timeoutError = new Error(
        `My Activity demorou mais de ${Math.round(timeoutMs / 1000)}s para responder.`,
      );
