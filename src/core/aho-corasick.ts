export type AhoCorasickPattern<T> = {
  id: string;
  pattern: string;
  value: T;
};

export type AhoCorasickMatch<T> = {
  id: string;
  pattern: string;
  value: T;
};
...
export class AhoCorasick<T> {
  ...
  private readonly patterns: AhoCorasickPattern<T>[] = [];

  constructor(patterns: AhoCorasickPattern<T>[]) {
    for (const pattern of patterns) {
      if (!pattern.id || !pattern.pattern) continue;
  ...
  private addPattern(pattern: AhoCorasickPattern<T>) {
