...
import type { ChatId, EvidenceSource, IsoDateTime, MetadataEvidence } from './types.js';
...
export type MetadataTextEvidenceInput = {
  source: Extract<EvidenceSource, 'my-activity-web' | 'takeout-html' | 'takeout-json'>;
  text?: string;
  date?: unknown;
  kind?: string;
  score?: number;
...
export type GroupedMetadataEvidence = {
...
export const sampleText = (value: unknown, max = 1200): string =>
  String(value || '')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, max);
...
export const assistantFragments = (value: unknown): string[] => {
...
const promptIsImageOnly = (value: unknown): boolean => {
...
export type CandidateNeedleOptions = {
  minLength?: number;
...
export const candidateNeedles = (
  candidate: MetadataCandidate,
  options: CandidateNeedleOptions = {},
): MetadataNeedle[] => {
  const needles: MetadataNeedle[] = [];
  const add = (
    kind: 'created' | 'last_message' | 'assistant' | 'title',
    value: unknown,
    weight: number,
    haystack?: MetadataNeedle['haystack'],
    requiresPromptComparable?: string,
  ) => {
    const text = sampleText(value, 500);
    const comparable = normalizeComparableText(text);
    const minLength =
      kind === 'created' || kind === 'last_message'
...
export const scoreMetadataEvidence = (
  candidate: MetadataCandidate,
  input: MetadataTextEvidenceInput,
  ...
  if (input.source === 'takeout-json' && input.kind) {
    return {
      chatId: String(candidate.chatId).toLowerCase() as ChatId,
      source: input.source,
      kind: input.kind,
  ...
  for (const needle of candidateNeedles(candidate)) {
    if (!comparableText.includes(needle.comparable)) continue;
    hits.push(needle);
    score += needle.weight;
  }
  ...
  return {
    ...
    sampleHash:
      input.sampleHash || hashText(hits.map((hit) => `${hit.kind}:${hit.text}`).join('\n')),
    sampleLength: input.sampleLength ?? String(input.text || '').length,
    warnings: [],
  };
};

const dateFromEvidence = (
  match: Partial<MetadataEvidence> & { timestamp?: unknown; time?: unknown },
) => portableIsoSeconds(match.date || match.timestamp || match.time);

export const groupMetadataEvidence = (
  matches: Array<Partial<MetadataEvidence> & { timestamp?: unknown; time?: unknown }> = [],
): Map<string, GroupedMetadataEvidence> => {
  const grouped = new Map<
    string,
    { created: IsoDateTime[]; last: IsoDateTime[]; evidence: MetadataEvidence[] }
  >();
  for (const match of matches) {
    ...
    const evidence: MetadataEvidence = {
      ...
      source: (match.source || 'takeout-html') as EvidenceSource,
      kind,
      dateKind: kind === 'created' || kind === 'last_message' ? kind : 'unknown',
    ...
    if (kind === 'created') current.created.push(date);
    else if (kind === 'last_message') current.last.push(date);
    grouped.set(chatId, current);
  }

  const result = new Map<string, GroupedMetadataEvidence>();
