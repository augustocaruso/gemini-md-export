...
type TabCommandState = {
  ...
  tabClaim?: Record<string, unknown> | null;
...
type SharedTabCommandOptions = {
  state?: TabCommandState;
  defaultReason: string;
  defaultClaimLabel: string;
  defaultClaimColor: string;
  ...
  getTabId?: () => number | null | undefined;
  getWindowId?: () => number | null | undefined;
  getTabClaim?: () => Record<string, unknown> | null | undefined;
  setTabClaim?: (claim: Record<string, unknown> | null) => void;
  rememberTabClaim?: (claim: Record<string, unknown>, response: Record<string, unknown>) => void;
  clearTabClaim?: () => void;
  setIsActiveTab?: (value: boolean) => void;
  releaseCurrentTabClaim?: (args: {
    claimId?: unknown;
    reason: string;
  }) => Promise<Record<string, unknown> | null | undefined>;
  afterReleaseByTabId?: (args: {
    command: BridgeCommand;
...
const commandArgs = (command: BridgeCommand): Record<string, unknown> => command.args || {};
...
const hasExplicitBrowserIntent = (args: Record<string, unknown>): boolean =>
  args.explicit === true ||
...
export const createSharedTabCommandHandlers = (options: SharedTabCommandOptions) => {
  ...
  const setTabClaim = (claim: Record<string, unknown> | null) => {
    if (options.setTabClaim) options.setTabClaim(claim);
    else if (options.state) options.state.tabClaim = claim;
  };
  const clearTabClaim = () => {
    if (options.clearTabClaim) options.clearTabClaim();
    else setTabClaim(null);
  };

  const execute = async (command: BridgeCommand) => {
    const args = commandArgs(command);
    if (command.type === 'get-extension-info') {
      if (options.getExtensionInfo) return options.getExtensionInfo(command);
    ...
    if (command.type === 'activate-tab' || command.type === 'activate-browser-tab') {
      ...
      if (
        typeof response?.isActiveTab === 'boolean' &&
        (requestedTabId === null || requestedTabId === localTabId)
      ) {
        if (options.setIsActiveTab) options.setIsActiveTab(response.isActiveTab);
        else if (options.state) options.state.isActiveTab = response.isActiveTab;
      }
    ...
    if (command.type === 'claim-tab') {
      ...
      if (!claimId) return { ok: false, reason: 'claim-id-required' };
      const claim = {
        claimId,
      ...
      if (response?.ok) {
        if (options.rememberTabClaim) options.rememberTabClaim(claim, response);
        else {
          setTabClaim({
            ...claim,
            tabId:
              response.tabId ??
              (response.visual as Record<string, unknown> | undefined)?.tabId ??
              getTabId(),
    ...
    if (command.type === 'release-tab-claim-by-tab-id') {
      ...
      const targetsThisTab =
        requestedTabId !== null && localTabId !== null && requestedTabId === localTabId;
      const claimMatches =
        !args.claimId || !localClaim?.claimId || localClaim.claimId === args.claimId;
      if (response?.ok && targetsThisTab && claimMatches) clearTabClaim();
