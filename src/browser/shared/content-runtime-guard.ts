...
export type ContentRuntimeBridgeStatus = {
  started?: boolean | null;
  heartbeatTimerActive?: boolean | null;
  heartbeatInFlight?: boolean | null;
  lastHeartbeatAt?: number | null;
  lastHeartbeatStartedAt?: number | null;
};

export type ContentRuntimeStatus = {
  ok?: boolean | null;
  installedAt?: number | null;
  bridge?: ContentRuntimeBridgeStatus | null;
};

export type ContentRuntimeGuardState =
  | { tag: 'checking' }
  | { tag: 'absent' }
  | { tag: 'signature-mismatch' }
  | { tag: 'same-signature-booting' }
  | { tag: 'same-signature-healthy' }
  | { tag: 'same-signature-stale' };

export type ContentRuntimeGuardEvent = {
  type: 'evaluate-runtime';
  hasRuntime: boolean;
  sameBuild: boolean;
  sameProtocol: boolean;
  status?: ContentRuntimeStatus | null;
  installedAt?: number | null;
  nowMs?: number;
  bootGraceMs?: number;
...
export type ContentRuntimeGuardEffect =
  | 'continue-bootstrap'
  | 'quiesce-existing'
  | 'return-existing';

export type ContentRuntimeGuardTransition = {
  state: ContentRuntimeGuardState;
  effects: ContentRuntimeGuardEffect[];
};
...
const withinAge = (nowMs: number, timestamp: number | null, maxAgeMs: number): boolean =>
  timestamp !== null && nowMs - timestamp >= 0 && nowMs - timestamp <= maxAgeMs;

export const classifyContentRuntimeHealth = ({
  status,
  installedAt,
  nowMs = Date.now(),
  bootGraceMs = CONTENT_RUNTIME_BOOT_GRACE_MS,
  staleHeartbeatMs = CONTENT_RUNTIME_HEARTBEAT_STALE_MS,
}: {
  status?: ContentRuntimeStatus | null;
  installedAt?: number | null;
  nowMs?: number;
  ...
  if (!bridge) return withinBootGrace ? 'booting' : 'stale';
  if (bridge.started !== true) return withinBootGrace ? 'booting' : 'stale';
...
export const transitionContentRuntimeGuard = (
  _state: ContentRuntimeGuardState,
  event: ContentRuntimeGuardEvent,
): ContentRuntimeGuardTransition => {
