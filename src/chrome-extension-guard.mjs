...
export const ensureChromeExtensionReady = async (deps, options = {}) => {
  ...
  while (true) {
    ...
    if (!reloadResult?.ok) {
      throw makeUserError(
        `Não consegui pedir reload automático da extensão do Chrome. Antes de repetir a exportação, cheque gemini_ready { action: "status", diagnostic: true } e gemini_tabs { action: "reload", intent: "tab_management" }; reload manual do card em chrome://extensions fica como fallback. Detalhe: ${reloadResult?.error || reloadResult?.reason || 'sem resposta'}`,
    ...
    if (!probe) {
      throw makeUserError(
        'Pedi reload automático da extensão do Chrome, mas ela não voltou a conectar. Rode gemini_ready { action: "status", diagnostic: true } para ver os clientes atuais e tente gemini_tabs { action: "reload", intent: "tab_management" } se houver abas conectadas. Reload manual do card da extensão em chrome://extensions só deve ser necessário se houve mudança de permissões/manifest ou se o navegador está apontando para uma pasta antiga.',
