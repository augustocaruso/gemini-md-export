...
export type BrowserAuthorityMcpLeaseResult = Readonly<{
  state: BrowserAuthorityState;
  lease: BrowserLease;
  token: ReturnType<typeof browserAuthorityLeaseToken>;
}>;
...
}>): BrowserAuthorityMcpLeaseResult => {
...
export const browserAuthorityBudgetForCommand = (
  commandType: string,
  nowMs = Date.now(),
): BrowserAuthorityBudget => {
...
export const browserAuthorityOperationKindForCommand = (
  commandType: string,
): BrowserAuthorityOperationKind => {
  if (commandType === 'claim-tab' || commandType === 'release-tab-claim') return 'tab_management';
  if (commandType === 'release-tab-claim-by-tab-id') return 'tab_management';
  if (commandType === 'reload-gemini-tabs' || commandType === 'reload-page')
    return 'tab_management';
  if (commandType === 'reload-extension-self') return 'ready_check';
  if (commandType === 'open-chat' || commandType === 'get-chat-by-id') return 'selected_export';
  return 'diagnostic';
};
...
export const assertBrowserAuthorityCommandAllowed = ({
  ...
  if (!mutatingBrowserCommandTypes.has(commandType)) return;
...
export const attachBrowserAuthorityLeaseToCommand = ({
  state,
  commandType,
  args,
  operationId,
...
export const prepareMcpBrowserAuthorityCommand = ({
  state,
  commandType,
  args,
  options = {},
  ...
  operationIdFallback?: unknown;
  nowMs?: number;
}>): Readonly<{
  state: BrowserAuthorityState;
  args: Record<string, unknown>;
  browserSideEffectExplicit: boolean;
}> => {
