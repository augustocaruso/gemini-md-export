...
type ReportDeps = {
  ...
  summarizeExportJobMetrics(job: AnyRecord, client: AnyRecord, options: AnyRecord): AnyRecord;
  summarizeTabSession(tabSession: unknown): AnyRecord | null;
  summarizeJobTrace(trace: unknown): AnyRecord;
  exportJobProgressMessage(job: AnyRecord): string;
  exportJobDecisionSummary(job: AnyRecord): AnyRecord;
  exportJobNextAction(job: AnyRecord): AnyRecord;
};

type RecentReportDeps = ReportDeps & {
  recentChatsExportScope(job: AnyRecord): AnyRecord;
};

type DirectReportDeps = ReportDeps & {
  directChatsExportScope(job: AnyRecord): AnyRecord;
};

const resumeChatIds = (resume: AnyRecord) => [...(resume.completedChatIds || [])];

export const buildRecentChatsExportReportPayload = (
  job: AnyRecord,
  client: AnyRecord,
  successes: AnyRecord[],
  failures: AnyRecord[],
  deps: RecentReportDeps,
) => ({
  job: {
    jobId: job.jobId,
    type: job.type,
...
export const buildDirectChatsExportReportPayload = (
  job: AnyRecord,
  client: AnyRecord,
  successes: AnyRecord[],
  failures: AnyRecord[],
  deps: DirectReportDeps,
) => ({
  job: {
    jobId: job.jobId,
    type: job.type,
  ...
  pendingItems: Array.isArray(job.pendingItems)
    ? job.pendingItems.map((item: AnyRecord) => ({
