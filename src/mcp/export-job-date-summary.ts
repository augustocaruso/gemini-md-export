...
export type DateImportMessageFsmState =
  | 'disabled'
  | 'my_activity_only'
  | 'takeout_only'
  | 'takeout_with_activity_fallback'
  | 'unknown_enabled';

export type DateImportMessageFsmResult = {
  state: DateImportMessageFsmState;
  sourceLabel: string;
  loadingMessage: string;
};

export type DateCompletenessGateFsmResult =
  | {
      state: 'disabled' | 'complete' | 'not_required';
      ok: true;
      status: 'completed';
      matched: number;
      partial: number;
...
const fixVaultCommandText = (job: AnyRecord = {}): string => {
  ...
  if (job.reportFile) parts.push('--report', shellQuote(job.reportFile));
  return parts.join(' ');
};

export const evaluateDateCompletenessGateFsm = (
  job: AnyRecord = {},
): DateCompletenessGateFsmResult => {
  ...
  const pendingText =
    `${counts.pending} conversa${counts.pending === 1 ? '' : 's'} ` +
  ...
  return {
    ...
    nextAction: {
      code: 'fix_vault_required',
      message:
        `Não consegui preencher todas as datas no ${sourceLabel}: ${pendingText}. ` +
...
export const terminalExportStatusForDateCompleteness = (
  job: AnyRecord = {},
  failureCount = 0,
...
export const dateCompletenessNextActionForJob = (
  job: AnyRecord = {},
): Extract<DateCompletenessGateFsmResult, { state: 'date_errors' }>['nextAction'] | null => {
  const decision = evaluateDateCompletenessGateFsm(job);
  return decision.state === 'date_errors' ? decision.nextAction : null;
};

export const dateImportLoadingMessageForJob = (job: AnyRecord = {}): string =>
  evaluateDateImportMessageFsm(job).loadingMessage;
...
export const exportJobProgressMessageForJob = (
  job: AnyRecord = {},
  context: {
  ...
  if (job.status === 'completed_with_errors') {
    ...
    if (dateAction) return dateAction.message;
