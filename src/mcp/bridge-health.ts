...
export type BridgeHealthDecision = Readonly<{
  status: BridgeHealthStatus;
  blockingIssue: string | null;
  action: string;
  clientKind: string;
  heartbeatAgeMs: number | null;
  runtimeSignalAgeMs: number | null;
  staleAfterMs: number;
  eventStreamConnected: boolean;
  longPollConnected: boolean;
  pagePolling: boolean | null;
  queuedCommands: number;
}>;

export type BridgeHealthOptions = Readonly<{
  now?: number;
  staleAfterMs: number;
  degradedHeartbeatMs: number;
...
const originForClient = (client: GeminiClientSnapshot): string | null => {
  try {
...
const clientIsActivity = (client: GeminiClientSnapshot) =>
  client.kind === 'activity' ||
  client.page?.kind === 'activity' ||
  originForClient(client) === 'https://myactivity.google.com';

const clientKind = (client: GeminiClientSnapshot) =>
  client.kind || client.page?.kind || (clientIsActivity(client) ? 'activity' : 'gemini');

const timestampAge = (timestamp: number | string | null | undefined, now: number) => {
  if (timestamp === null || timestamp === undefined || timestamp === '') return null;
  const numeric = Number(timestamp);
  return Number.isFinite(numeric) ? now - numeric : null;
};

const runtimeSignalAge = (client: GeminiClientSnapshot, now: number) => {
...
export const clientPageBlockerCode = (client: GeminiClientSnapshot): string | null => {
  const code = String(client.page?.blocker?.code || '').trim();
  if (code) return code;
  return client.page?.blocker?.terminal === true ? 'google_page_blocked' : null;
};

export const clientHasPageBlocker = (client: GeminiClientSnapshot) =>
  client.page?.blocker?.terminal === true;

export const evaluateBridgeHealth = (
  client: GeminiClientSnapshot,
  options: BridgeHealthOptions,
): BridgeHealthDecision => {
  ...
  let status: BridgeHealthStatus = 'healthy';
