...
type Dependencies = Readonly<{
  runDoctor: (parsed: Parsed, streams: Streams) => Promise<{ exitCode: number; result: any }>;
  runFixVault: (parsed: Parsed, streams: Streams) => Promise<{ exitCode: number; result: any }>;
}>;

export const runFixVaultSmokeCommand = async ({
  parsed,
  streams = {},
  dependencies,
}: {
  ...
  if (doctor.exitCode !== 0) {
    ...
    if (parsed.flags.format !== 'json') stdout.write('Smoke: bloqueado no doctor.\n');
