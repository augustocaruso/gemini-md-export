...
import {
  type GeminiWebapiPythonListChatsResult,
...
import {
  type PrivateApiSelectedExportItem,
...
export type PrivateApiSyncInventoryItem = PrivateApiSelectedExportItem;

export type PrivateApiSyncDecision = Readonly<{
  inventory: readonly PrivateApiSyncInventoryItem[];
  boundary: Readonly<{
    found: boolean;
    type: 'sync-state-boundary' | 'known-vault-sequence' | 'end-of-private-inventory' | 'not-found';
    chatId: string | null;
    index: number | null;
    knownSequenceLength: number;
  }>;
  itemsToExport: readonly PrivateApiSyncInventoryItem[];
  skippedExisting: number;
  existingInVaultBeforeBoundary: number;
}>;

export type PrivateApiSyncJob = Readonly<{
  jobId: string;
  type: 'private-api-sync';
  sourceKind: 'export-job';
  status: 'running' | 'completed' | 'completed_with_errors' | 'failed';
  phase: 'listing' | 'exporting' | 'writing-report';
  requested: number;
  completed: number;
  batchTotal: number;
  successCount: number;
  ...
  decisionSummary: Readonly<{
    ...
    boundary: PrivateApiSyncDecision['boundary'];
  ...
  savedFiles: PrivateApiSelectedExportJob['savedFiles'];
  failures: PrivateApiSelectedExportJob['failures'];
  syncStateFile: string;
  startedAt: string;
  updatedAt: string;
  finishedAt?: string;
...
export type PrivateApiSyncArgs = Readonly<{
  vaultDir?: unknown;
  outputDir?: unknown;
  syncStateFile?: unknown;
  maxChats?: unknown;
  knownBoundaryCount?: unknown;
  waitMs?: unknown;
  ...
  onProgress?: (job: PrivateApiSyncJob) => void;
}>;

type PrivateApiSyncDeps = Readonly<{
  now(): Date;
  listChats(input: {
    limit: number;
    timeoutMs?: unknown;
    python?: unknown;
    cookiesJson?: unknown;
  }): Promise<AnyRecord | GeminiWebapiPythonListChatsResult>;
  exportSelected(input: AnyRecord): Promise<PrivateApiSelectedExportJob>;
}>;
...
const resolvePathInput = (value: unknown, fallback?: string): string => {
...
export const resolvePrivateSyncStateFile = (vaultDir: string, syncStateFile?: unknown): string => {
...
const readJsonFile = (filePath: string): AnyRecord | null => {
  try {
...
const chatIdFromLooseText = (value: unknown): string | null => {
...
const collectVaultChatIds = (rootDir: string, out = new Set<string>()): Set<string> => {
  for (const entry of readdirSync(rootDir, { withFileTypes: true })) {
    if (entry.isDirectory()) {
      if (INTERNAL_DIRS.has(entry.name.toLowerCase())) continue;
    ...
    const fromFrontmatter =
      chatIdFromLooseText(frontmatterField(text, 'chat_id')) ||
      chatIdFromLooseText(frontmatterField(text, 'url'));
    if (fromFrontmatter) out.add(fromFrontmatter);
  }
  return out;
};

export const scanPrivateSyncVault = (vaultDir: string) => {
...
const normalizeInventoryChat = (chat: AnyRecord): PrivateApiSyncInventoryItem | null => {
...
const normalizeInventory = (result: AnyRecord): PrivateApiSyncInventoryItem[] => {
  ...
  const items: PrivateApiSyncInventoryItem[] = [];
...
const syncStateBoundaryIds = (syncState: AnyRecord | null | undefined): Set<string> => {
...
export const selectPrivateSyncItems = ({
  inventory,
  existingChatIds,
  syncState,
  knownBoundaryCount,
  ...
  syncState?: AnyRecord | null;
  knownBoundaryCount?: unknown;
  maxChats?: unknown;
}>): PrivateApiSyncDecision => {
  ...
  let knownRunStart: number | null = null;
  let knownRunLength = 0;
  let boundary: PrivateApiSyncDecision['boundary'] = {
  ...
  const existingInVaultBeforeBoundary = beforeBoundary.filter((item) =>
    existingChatIds.has(item.chatId),
  ).length;
  return {
    inventory,
    boundary,
...
const privateInventoryLimit = (args: PrivateApiSyncArgs): number => {
  const maxChats = numberInRange(args.maxChats, 0, 0, 2000);
  const knownBoundaryCount = numberInRange(
    args.knownBoundaryCount,
    DEFAULT_KNOWN_BOUNDARY_COUNT,
    1,
    100,
  );
...
const listChatsViaNativeOrPython = async (input: {
  ...
  if (!stringOrNull(input.python) && !stringOrNull(input.cookiesJson)) {
    try {
      const nativeResponse = (await createNativeBrowserBrokerClient().privateApiListChats(
        { limit: input.limit, timeoutMs },
        { allowFallback: true },
      )) as AnyRecord;
...
const defaultDeps: PrivateApiSyncDeps = {
  now: () => new Date(),
...
const makeJob = ({
  ...
  args: PrivateApiSyncArgs;
  status: PrivateApiSyncJob['status'];
  phase: PrivateApiSyncJob['phase'];
  outputDir: string;
  syncStateFile: string;
  inventory: readonly PrivateApiSyncInventoryItem[];
  inventorySource: string | null;
  vaultChatIdCount: number;
  decision: PrivateApiSyncDecision;
  exportJob?: PrivateApiSelectedExportJob | null;
  progressMessage: string;
  startedAt: string;
  updatedAt: string;
  finishedAt?: string;
  ...
  const fullHistoryVerified =
    decision.boundary.found === true || inventory.length < privateInventoryLimit(args);
  return {
...
const statusFromExport = (
  exportJob: PrivateApiSelectedExportJob | null,
  fullHistoryVerified: boolean,
): PrivateApiSyncJob['status'] => {
  if (!exportJob) return fullHistoryVerified ? 'completed' : 'completed_with_errors';
  if (exportJob.status === 'failed') return 'failed';
  if (exportJob.failureCount > 0 || !fullHistoryVerified) return 'completed_with_errors';
  return 'completed';
};
...
export const runPrivateApiSync = async (
  args: PrivateApiSyncArgs,
  deps: Partial<PrivateApiSyncDeps> = {},
): Promise<PrivateApiSyncJob> => {
  const runtimeDeps: PrivateApiSyncDeps = { ...defaultDeps, ...deps };
  ...
  const emptyDecision: PrivateApiSyncDecision = {
    inventory: [],
    boundary: {
      found: false,
      type: 'not-found',
  ...
  if (listResult?.ok !== true) {
    const err = Object.assign(
      new Error(
        stringOrNull((listResult as AnyRecord)?.message) ||
          stringOrNull((listResult as AnyRecord)?.error) ||
          'Nao consegui listar conversas pela API privada.',
      ),
      {
        code: stringOrNull((listResult as AnyRecord)?.code) || 'private_inventory_unavailable',
        data: listResult,
      },
    );
    throw err;
  ...
  const inventory = normalizeInventory(listResult as AnyRecord);
  ...
  const inventorySource = stringOrNull((listResult as AnyRecord).source);
  const preExportJob = makeJob({
    ...
    decision,
    progressMessage:
      decision.itemsToExport.length === 0
  ...
  let exportJob: PrivateApiSelectedExportJob | null = null;
  if (decision.itemsToExport.length > 0) {
    exportJob = await runtimeDeps.exportSelected({
      ...
      onProgress: (selectedJob: PrivateApiSelectedExportJob) => {
        args.onProgress?.(
          makeJob({
            args,
            status: 'running',
...
export const summarizePrivateApiSyncJob = (job: PrivateApiSyncJob) => ({
