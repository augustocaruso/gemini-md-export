...
import type {
  ...
  PrivateApiSelectedExportItem,
  PrivateApiSelectedExportJob,
  PrivateApiSelectedExportSavedFile,
} from './private-api-selected-export.js';

type AnyRecord = Record<string, any>;

type BridgePrivateExportArgs = Readonly<{
  bridgeUrl: unknown;
    ...
    throw Object.assign(
      new Error(
        err instanceof Error
          ...
          : 'Bridge local indisponivel para export privado.',
      ),
      { code: 'bridge_private_export_unavailable', cause: err },
    );
  } finally {
...
const readJsonFileIfExists = (filePath: unknown): AnyRecord | null => {
  const resolved = stringOrNull(filePath);
  if (!resolved || !existsSync(resolved)) return null;
  try {
    return JSON.parse(readFileSync(resolved, 'utf-8')) as AnyRecord;
  } catch {
    return null;
  }
};

const bridgeStatusToPrivateStatus = (status: unknown): PrivateApiSelectedExportJob['status'] => {
...
const bridgePhaseToPrivatePhase = (phase: unknown): PrivateApiSelectedExportJob['phase'] =>
  phase === 'writing-report' ? 'writing-report' : 'exporting';

const savedFilesFromReport = (
  ...
  const successes = Array.isArray(report?.successes)
    ...
    : Array.isArray(job.recentSuccesses)
      ? job.recentSuccesses
      : [];
  return successes.map((item: AnyRecord) => ({
...
const failuresFromReport = (
  ...
  const failures = Array.isArray(report?.failures)
    ...
    : Array.isArray(job.failures)
      ? job.failures
      : [];
  return failures.map((item: AnyRecord, index: number) => ({
...
const currentFromBridgeJob = (
  job: AnyRecord,
  requested: number,
): PrivateApiSelectedExportJob['current'] => {
...
const toPrivateApiSelectedExportJob = ({
  bridgeJob,
  report,
  fallbackOutputDir,
  fallbackRequested,
  ...
  const requested = numberInRange(
    bridgeJob.requested ?? report?.requested ?? fallbackRequested,
    Math.max(fallbackRequested, savedFiles.length + failures.length),
    0,
    2000,
  );
  const completed = numberInRange(
    bridgeJob.completed ?? savedFiles.length + failures.length,
    savedFiles.length + failures.length,
    0,
    requested,
  );
...
const requestBodyForBridgePrivateExport = (args: BridgePrivateExportArgs): AnyRecord => ({
...
export const runPrivateApiSelectedExportViaBridge = async (
  args: BridgePrivateExportArgs,
): Promise<PrivateApiSelectedExportJob> => {
  if (args.items.length === 0) {
    throw Object.assign(
      new Error('Nenhuma conversa selecionada para export privado pela bridge.'),
      {
  ...
  const expectedCount = numberInRange(
    args.expectedCount ?? args.items.length,
    args.items.length,
    0,
    2000,
  );
  ...
  while (!TERMINAL_STATUSES.has(String(bridgeJob.status))) {
    ...
    bridgeJob = await jsonFetch(
      args.bridgeUrl,
      appendParams('/agent/export-job-status', { jobId: privateJob.jobId }),
      { timeoutMs: 20_000 },
    );
