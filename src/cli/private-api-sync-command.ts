...
import {
  type PrivateApiSyncJob,
  runPrivateApiSync,
  summarizePrivateApiSyncJob,
} from './private-api-sync.js';
...
const terminal = (job: PrivateApiSyncJob): boolean =>
...
const renderTui = (stdout: NonNullable<Streams['stdout']>, job: PrivateApiSyncJob) => {
  const width = Math.max(18, Math.min(42, Math.floor((stdout.columns || 88) / 3)));
  const done = Math.min(
    job.requested,
    job.successCount + job.failureCount + (job.current && !terminal(job) ? 1 : 0),
  );
...
const renderPlain = (stdout: Writable, job: PrivateApiSyncJob, previousKey: string) => {
...
const normalizeSyncFlags = ({ flags, parsed }: Pick<CommandInput, 'flags' | 'parsed'>) => {
  const normalized = { ...(flags || parsed?.flags || {}) };
  if (normalized.allowReloadExplicit !== true) normalized.allowReload = true;
  if (normalized.activateTabExplicit !== true) normalized.activateTab = true;
  if (!normalized.vaultDir && parsed?.positionals?.[0]) normalized.vaultDir = parsed.positionals[0];
...
const normalizeCommandInput = (
  input: CommandInput | CommandInput['parsed'],
  streams?: Streams,
  ensureBridgeAvailable?: NonNullable<CommandInput['dependencies']>['ensureBridgeAvailable'],
): CommandInput => {
...
export const runPrivateApiSyncCommand = async (
  input: CommandInput | CommandInput['parsed'],
  positionalStreams?: Streams,
  positionalEnsureBridgeAvailable?: NonNullable<
    CommandInput['dependencies']
  >['ensureBridgeAvailable'],
) => {
  const {
    flags: inputFlags,
    ...
    streams = {},
    dependencies = {},
  } = normalizeCommandInput(input, positionalStreams, positionalEnsureBridgeAvailable);
  const flags = normalizeSyncFlags({ flags: inputFlags, parsed });
  const stdout = streams.stdout || process.stdout;
  const format = selectFormat(flags, stdout);
  let latestJob: PrivateApiSyncJob | null = null;
  ...
  const onProgress = (job: PrivateApiSyncJob) => {
    latestJob = job;
