...
export const connectedClientCountFromReady = (ready: unknown = {}): number => {
  const record = asRecord(ready);
  return (
    Number(
      record.connectedClientCount ?? record.connectedClients?.length ?? record.clients?.length ?? 0,
    ) || 0
  );
};

export const readinessIssueCode = (issue: unknown): string => {
  if (!issue) return '';
...
export const readinessIssueMessage = (issue: unknown): string => {
  if (!issue) return '';
  if (typeof issue === 'string') return issue;
  if (typeof issue === 'object') {
    const record = issue as AnyRecord;
...
export const readinessIssueDisplay = (issue: unknown, fallback = 'desconhecido'): string =>
  readinessIssueCode(issue) || readinessIssueMessage(issue) || fallback;

export const browserDiagnosticMessage = (diagnosis: AnyRecord = {}): string | null => {
...
export const browserDiagnosticBlockingIssue = (diagnosis: AnyRecord = {}): string | null => {
...
export const attachBrowserDiagnosticToReady = (
  ready: AnyRecord = {},
  browserTabs: AnyRecord = {},
  { forceGemini = false }: { forceGemini?: boolean } = {},
): AnyRecord => {
  ...
  const effectiveDiagnosis =
    forceGemini && diagnosis.kind === 'gemini' ? { ...diagnosis, terminal: true } : diagnosis;
