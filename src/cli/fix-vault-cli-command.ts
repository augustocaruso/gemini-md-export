...
type Streams = Readonly<Record<string, unknown>>;

type CommandResult = Promise<{ exitCode: number; result: any }>;

export const buildFixVaultCliHelp = ({
  commonOptions = [],
}: {
...
export const normalizeFixVaultCliParsed = (parsed: Parsed): Parsed => ({
  ...parsed,
  flags: applyPrivateApiSessionDefaults(parsed.flags),
});

export const runFixVaultCliCommand = ({
  parsed,
  streams,
  runDoctor,
  runDefault,
  ...
  parsed: Parsed;
  streams: Streams;
  runDoctor: (parsed: Parsed, streams: Streams) => CommandResult;
  runDefault: (parsed: Parsed, streams: Streams) => CommandResult;
}): CommandResult => {
  const normalized = normalizeFixVaultCliParsed(parsed);
  const subcommand = normalized.positionals[0];
  if (subcommand === 'doctor') return runDoctor(normalized, streams);
  if (subcommand === 'smoke') {
    return runFixVaultSmokeCommand({
      parsed: normalized,
      streams,
      dependencies: {
        runDoctor: (nextParsed, nextStreams) =>
          runDoctor(normalizeFixVaultCliParsed(nextParsed), nextStreams),
        runFixVault: (nextParsed, nextStreams) =>
          runDefault(normalizeFixVaultCliParsed(nextParsed), nextStreams),
      },
    });
  }
