...
const argValue = (args: readonly string[], name: string, fallback = ''): string => {
...
export const nativeHostManifestUsage = (): string =>
  [
...
const templatePathForRoot = (root: string): string =>
  resolve(root, 'native-messaging', 'com.augustocaruso.gemini_md_export.template.json');

const installedRootForHome = (homeDir: string): string =>
  resolve(homeDir, '.gemini', 'extensions', 'gemini-md-export');

const shellDoubleQuote = (value: string): string =>
  `"${String(value).replace(/["\\$`]/g, '\\$&')}"`;

const csharpStringLiteral = (value: string): string =>
  `"${String(value).replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`;

const powershellSingleQuote = (value: string): string => `'${String(value).replace(/'/g, "''")}'`;
...
export const macNativeHostDir = (homeDir: string, browser: string): string => {
  const appName =
    browser === 'edge'
      ? 'Microsoft Edge'
      : browser === 'brave'
...
export const windowsRegistryKey = (browser: string): string => {
  if (browser === 'edge')
  ...
  if (browser === 'dia') return `HKCU\\Software\\Dia\\NativeMessagingHosts\\${NATIVE_HOST_NAME}`;
  return `HKCU\\Software\\Google\\Chrome\\NativeMessagingHosts\\${NATIVE_HOST_NAME}`;
};

const resolveNativeHostPaths = ({
  root,
  homeDir,
  env,
  currentPlatform,
  ...
  const defaultRoot =
    !env.GEMINI_MD_EXPORT_NATIVE_HOST_PATH &&
  ...
  const windowsLauncherPath =
    env.GEMINI_MD_EXPORT_NATIVE_HOST_LAUNCHER_PATH ||
  ...
  return {
    ...
    nativeHostPath:
      currentPlatform === 'win32' && !env.GEMINI_MD_EXPORT_NATIVE_HOST_PATH
        ? windowsLauncherPath
        : runtimePath,
  };
};

const renderMacLauncher = (nodePath: string, runtimePath: string): string =>
  [
    '#!/bin/sh',
...
const writeMacLauncherIfNeeded = ({
  launcherPath,
  nodePath,
  runtimePath,
}: {
...
const writeWindowsLauncher = ({
  ...
  const command = [
    ...
    `$source = @'\n${source}\n'@`,
    `Add-Type -TypeDefinition $source -Language CSharp -OutputAssembly ${powershellSingleQuote(
      launcherPath,
...
const buildManifest = ({
  ...
  return {
    ...manifestTemplate,
    path: nativeHostPath,
    allowed_origins: (manifestTemplate.allowed_origins || []).map((origin: string) =>
      String(origin).replace(/__EXTENSION_ID__/g, extensionId),
    ),
  };
};

const print = (stdout: NodeJS.WritableStream, value: string): void => {
  stdout.write(`${value}\n`);
};

export const runNativeHostManifestCli = ({
  argv,
  root,
  homeDir = homedir(),
  platform = osPlatform(),
  ...
  if (platform === 'darwin') {
    const launcherPath = resolve(
      dirname(paths.runtimePath),
      'gemini-md-export-native-host-launcher.sh',
    );
  ...
  if (platform === 'win32') {
    ...
    const result = spawnSync(
      'reg.exe',
      ['add', windowsRegistryKey(browser), '/ve', '/t', 'REG_SZ', '/d', target, '/f'],
      {
        stdio: 'inherit',
      },
    );
