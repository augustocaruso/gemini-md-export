...
const WINDOWS_SHELL_EXECUTE_SCRIPT_FILENAME = 'open-gemini-shell-execute.ps1';
...
const windowsShellExecuteLaunchPreferred = (env = process.env) => {
  const method = String(
    env.GEMINI_MCP_BROWSER_LAUNCH_METHOD || env.GME_BROWSER_LAUNCH_METHOD || '',
  )
    .trim()
    .toLowerCase();
  if (/shell[-_\s]*execute/.test(method)) return true;
  if (/restore[-_\s]*focus|direct/.test(method)) return false;
  return Boolean(env.SSH_CONNECTION || env.SSH_CLIENT);
};

export const buildWindowsShellExecuteLaunchScript = (command, args = []) => {
  const argArray = args.map(quotePowerShellString).join(', ');
  return [
    '# Generated by gemini-md-export. Opens Gemini through the Windows shell so SSH/non-interactive sessions reach the user desktop.',
    "$ErrorActionPreference = 'Stop'",
    ...
    `$browser = ${quotePowerShellString(command)}`,
    `$arguments = @(${argArray})`,
    "$parameterText = (($arguments | ForEach-Object { Quote-CommandLineArgument $_ }) -join ' ')",
    '$shell = New-Object -ComObject Shell.Application',
    "$shell.ShellExecute($browser, $parameterText, '', 'open', 1) | Out-Null",
    '',
  ].join('\r\n');
};

export const writeWindowsShellExecuteLaunchScript = (
  command,
  args = [],
  { env = process.env } = {},
) => {
  const dir = browserLaunchStateDir(env);
  mkdirSync(dir, { recursive: true });
  const scriptPath = resolve(dir, WINDOWS_SHELL_EXECUTE_SCRIPT_FILENAME);
  writeFileSync(scriptPath, buildWindowsShellExecuteLaunchScript(command, args), 'utf-8');
  return scriptPath;
};
...
export const launchGeminiBrowser = async ({
  ...
  try {
    ...
    if (platform === 'win32') {
      ...
      if (windowsShellExecuteLaunchPreferred(env)) {
        let shellExecuteScriptPath = null;
        try {
          shellExecuteScriptPath = writeWindowsShellExecuteLaunchScript(plan.binary, args, { env });
          const shellExecute = await observeDetachedSpawn(spawnFn, 'powershell.exe', [
            '-NoProfile',
            '-ExecutionPolicy',
            'Bypass',
            '-WindowStyle',
          ...
          if (shellExecute.ok) {
            return {
              ...
              supported: true,
              ...plan,
              method: 'windows-powershell-shell-execute',
              targetUrl: launchUrl,
              reusedExistingTab: false,
              openedNewTab: true,
              profileDirectory: resolvedProfileDirectory,
      ...
      return {
        ...
        error: direct.error || `exit ${direct.exitCode ?? 'unknown'}`,
      };
    }
